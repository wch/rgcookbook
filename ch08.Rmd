...
In ggplot, there are two ways of setting the range of the axes. The first way is to modify the *scale*, and the second is to apply a *coordinate transform*. When you modify the limits of the *x* or *y* scale, any data outside of the limits is removed -- that is, the out-of-range data is not only not displayed, it is removed from consideration entirely.
...
TODO: Add note explaining the warning here.
...
library(gcookbook)  # Load the gcookbook package to access the marathon data set 
...
(ref:cap-FIG-AXES-AXIS-LABEL-REMOVE) X-axis label with `element_blank()` (left); With the label set to `""` (right)
...
library(MASS) # Load the MASS package to access the Animals data set 
...
```{r FIG-AXES-LOG-BREAKS, ref.label=c("FIG-AXES-LOG-BREAKS-1", "FIG-AXES-LOG-BREAKS-2"), echo=FALSE, message=FALSE, fig.show="hold", fig.cap="Scatter plot with log~10~ x- and y-axes, and with manually specified breaks (left); With exponents for the tick labels (right)"}
```
...
library(gcookbook)  # Load the gcookbook package to access the aapl data set 
...
```{r}
library(gcookbook)  # Load the gcookbook package to access the wind data set 
wind
```
...
# Create the base plot
mdeaths_plot <- 
  ggplot(mdeaths_mod, aes(x = month, y = deaths)) +
  geom_line() +
