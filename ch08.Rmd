...
If the variable is continuous, see [section\_title](#RECIPE_AXES_REVERSE) to reverse the direction.
...
To reverse the order of items on a *discrete* axis, see [section\_title](#RECIPE_AXIS_ORDER).
...
```{r FIG-AXES-ORDER-MANUAL, fig.show="hold", fig.cap="Left: box plot with manually specified items on the x-axis; right: with only two items"}
p <- ggplot(PlantGrowth, aes(x = group, y = weight)) + geom_boxplot()
...
To reorder factor levels based on data values from another column, see [???](#RECIPE_DATAPREP_FACTOR_REORDER_VALUE).
...
To remove the tick marks and labels (but not the data) from thegraph, see [section\_title](#RECIPE_AXIS_REMOVE_TICKS).
...
This will work for continuous axes only; if you remove items from a categorical axis using limits, as in [section\_title](#RECIPE_AXIS_ORDER), the data with that value won't be shown at all.
...
These commands control the appearance of only the tick labels, on only one axis. They don't affect the other axis, the axis label, the overall title, or the legend. To control all of these at once, you can use the theming system, as discussed in [???](#RECIPE_APPEARANCE_THEME).

### See Also

See [???](#RECIPE_APPEARANCE_TEXT_APPEARANCE) for more about controlling the appearance of the text.
...
```{r FIG-AXES-AXIS-LABEL-REMOVE, ref.label=c("FIG-AXES-AXIS-LABEL-REMOVE-1", "FIG-AXES-AXIS-LABEL-REMOVE-2"), echo=FALSE, fig.show="hold", fig.cap='Left: x-axis label with element_blank(); right: with the label set to ""', fig.width=3.5, fig.height=3.5}
```
...
```{r FIG-AXES-AXIS-LINE, ref.label=c("FIG-AXES-AXIS-LINE-1", "FIG-AXES-AXIS-LINE-2"), echo=FALSE, fig.show="hold", fig.cap="Left: scatter plot with axis lines; right: with theme_bw(), panel.border must also be made blank"}
```
...
For more information about how the theming system works,see [???](#RECIPE_APPEARANCE_THEME).
...
Making a Circular Plot {#RECIPE_AXES_POLAR}
-----------------------
...
See [???](#RECIPE_LEGEND_REVERSE) for more about reversing the direction of a legend.

See [section\_title](#RECIPE_AXES_SET_TICKS) for more about specifying which values will have tick marks (breaks) and labels.
...
See [???](#RECIPE_DATAPREP_TIMESERIES) for information about converting time series objects to data frames.
