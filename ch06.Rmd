...
variable, `smoke`, is stored as a number, so we’ll use the `birthwt1` data set we
...
(ref:cap-FIG-DISTRIBUTION-DENSITY-BASIC) A kernel density estimate curve
with `geom_density()` (left); With `geom_line()` (right)
...
frame. For this example, we’ll use the `faithful` data set, which contains two
...
eruption. We’ll only use the `waiting` column in this example:
