...
```{r FIG-DISTRIBUTION-HIST-WIDTH, fig.show="hold", fig.cap="Left: histogram with binwidth=5, and with different colors; right: with 15 bins", fig.width=3.5, fig.height=3.5}
...
```{r FIG-DISTRIBUTION-MULTI-HISTOGRAM-FACET, fig.cap="Left: two histograms with facets; right: with different facet labels"}
...
```{r FIG-DISTRIBUTION-DENSITY-BASIC, ref.label=c("FIG-DISTRIBUTION-DENSITY-BASIC-1", "FIG-DISTRIBUTION-DENSITY-BASIC-2"), echo=FALSE, fig.show="hold", fig.cap="Left: a kernel density estimate curve with geom_density(); right: with geom_line()", fig.width=3.5, fig.height=3.5}
```
...
```{r FIG-DISTRIBUTION-DENSITY-WIDTH, fig.show="hold", fig.cap="Left: density curve with wider x limits and a semitransparent fill; right: in two parts, with geom_density() and geom_line()", fig.width=3.5, fig.height=3.5}
ggplot(faithful, aes(x = waiting)) +
...
See [section\_title](#RECIPE_DISTRIBUTION_VIOLIN) for information on violin plots, which are another way of representing density curves and may be more appropriate for comparing multiple distributions.
...
```{r FIG-DISTRIBUTION-FREQPOLY, fig.cap="Left: a frequency polygon; right: with wider bins"}
...
[TODO: add comparison of freqpoly to histogram. Pro: ?? Con: Can't know for sure how wide bins are]
...
```{r FIG-DISTRIBUTION-BOXPLOT-DIAGRAM, echo=FALSE, fig.cap="Box plot compared to histogram and density curve", figh.width=7, fig.height=3}
set.seed(122)
...
ggplot(ds, aes(x=x)) + 
    ...
    annotate("text", x=sumx[["Min."]], y=0, label="Minimum",
             angle=90, vjust=-0.2, hjust=0, size=4) +
    annotate("text", x=sumx[["1st Qu."]], y=0, label="25th percentile",
             angle=90, vjust=-0.2, hjust=0, size=4) +
    annotate("text", x=sumx[["Median"]], y=0,  label="Median",
             angle=90, vjust=-0.2, hjust=0, size=4) +
    annotate("text", x=sumx[["3rd Qu."]], y=0, label="75th percentile",
             angle=90, vjust=-0.2, hjust=0, size=4) +
    geom_segment(aes(x=sumx[["Min."]], xend=sumx[["1st Qu."]], y=.75, yend=.75),
                 size=.2, arrow=arrow(ends="both", length=unit(0.2,"cm"))) +
    annotate("text", x=mean(c(sumx[["Min."]], sumx[["1st Qu."]])), y=.75, vjust=-0.2, size=4, 
             label="To minimum", lineheight=.8) +
    geom_segment(aes(x=sumx[["1st Qu."]], xend=sumx[["3rd Qu."]], y=.85, yend=.85),
                 size=.2, arrow=arrow(ends="both", length=unit(0.2,"cm"))) +
    annotate("text", x=mean(c(sumx[["1st Qu."]], sumx[["3rd Qu."]])), y=.85, vjust=-0.2, size=4,
             label="IQR") +
    geom_segment(aes(x=sumx[["3rd Qu."]], xend=sumx[["3rd Qu."]] + 1.5*iqr, y=.75, yend=.75),
                 size=.2, arrow=arrow(ends="both", length=unit(0.2,"cm"))) +
    annotate("text", x=sumx[["3rd Qu."]] + .75*iqr, y=.75, vjust=-0.2, size=4,
             label="1.5 x IQR") +
    theme_bw() +
