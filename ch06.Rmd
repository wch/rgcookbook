...
Use geom_histogram() andmap a continuous variable to x
(Figure \@ref(fig:FIG-DISTRIBUTION-HIST-BASIC)):
...
All geom_histogram() requires is one column from a data frame
...
Figure \@ref(fig:FIG-DISTRIBUTION-HIST-WIDTH).
...
Figure \@ref(fig:FIG-DISTRIBUTION-HIST-QUANTIZE), we'll use a bin
...
It is also possible to use geom_bar(stat="bin") for the same
...
Use geom_histogram() anduse facets for each group, as shown in
Figure \@ref(fig:FIG-DISTRIBUTION-MULTI-HISTOGRAM-FACET):
...
(Figure \@ref(fig:FIG-DISTRIBUTION-MULTI-HISTOGRAM-FACET), right).
...
(Figure \@ref(fig:FIG-DISTRIBUTION-MULTI-HISTOGRAM-FACET-SCALESFREE),
left):
...
Figure \@ref(fig:FIG-DISTRIBUTION-MULTI-HISTOGRAM-FILL). The grouping
...
Like geom_histogram(), geom_density() requires just
...
faithful
eruptions waiting 3.600 79 1.800 54 3.333 74 ...
The second method mentioned earlier uses geom_line() and tells
it to use the "density" statistical transformation. This is essentially
the same as the first method, using geom_density(), except the
former draws it with a closed polygon.

As with geom_histogram(), if you just want to get a quick look
...
value of 1. Figure \@ref(fig:FIG-DISTRIBUTION-DENSITY-ADJUST) shows what
...
(Figure \@ref(fig:FIG-DISTRIBUTION-DENSITY-WIDTH)). We'll also add
...
we'll add geom_histogram() first, and then layer
geom_density() on top
(Figure \@ref(fig:FIG-DISTRIBUTION-DENSITY-HIST)):
...
Use geom_density(), andmap the grouping variable to an
aesthetic like colour or fill, as shown in
Figure \@ref(fig:FIG-DISTRIBUTION-MULTI-DENSITY). The grouping variable
...
shown in Figure \@ref(fig:FIG-DISTRIBUTION-MULTI-DENSITY-FACET). We can
...
(Figure \@ref(fig:FIG-DISTRIBUTION-MULTI-DENSITY-HIST)):
...
polygon (Figure \@ref(fig:FIG-DISTRIBUTION-FREQPOLY), right):
...
Use geom_boxplot(),mapping a continuous variable to y and a
discrete variable to x
(Figure \@ref(fig:FIG-DISTRIBUTION-BOXPLOT-BASIC)):
...
low age lwt race smoke ptl ht ui ftv bwt 0 19 182 2 0 0 0 1 0 2523 0 33
155 3 0 0 0 0 3 2551 0 20 105 1 1 0 0 0 1 2557 ...
In Figure \@ref(fig:FIG-DISTRIBUTION-BOXPLOT-BASIC), the data is divided
...
Figure \@ref(fig:FIG-DISTRIBUTION-BOXPLOT-DIAGRAM) shows the
...
(Figure \@ref(fig:FIG-DISTRIBUTION-BOXPLOT-WIDTH-POINT), left):
...
(Figure \@ref(fig:FIG-DISTRIBUTION-BOXPLOT-WIDTH-POINT), right):
...
(Figure \@ref(fig:FIG-DISTRIBUTION-BOXPLOT-SINGLE)):
...
> small sample sizes. See ?geom_boxplot for detailed
...
Use stat_summary(). Themean is often shown with a diamond, so
we'll use shape 23 with a white fill. We'll also make the
diamond slightly larger by setting size=3
(Figure \@ref(fig:FIG-DISTRIBUTION-BOXPLOT-MEAN)):
...
Use geom_violin()
(Figure \@ref(fig:FIG-DISTRIBUTION-VIOLIN-BASIC)):
...
Figure \@ref(fig:FIG-DISTRIBUTION-VIOLIN-BOXPLOT). Additionally, thebox
...
is calculated using stat_summary().
...
(Figure \@ref(fig:FIG-DISTRIBUTION-VIOLIN-TAIL)):
...
group (Figure \@ref(fig:FIG-DISTRIBUTION-VIOLIN-SCALECOUNT)). In this
...
(Figure \@ref(fig:FIG-DISTRIBUTION-VIOLIN-ADJUST)):
...
Use geom_dotplot(). Forthis example
(Figure \@ref(fig:FIG-DISTRIBUTION-DOTPLOT-BASIC)), we'll use a
...
By default, geom_dotplot() bins the data along the x-axis and
...
removed by using scale_y\$\$\_\$\$continuous(). In this
example, we'll also use geom_rug() to show exactly where
each data point is
(Figure \@ref(fig:FIG-DISTRIBUTION-DOTPLOT-NO-Y-RUG)):
...
Figure \@ref(fig:FIG-DISTRIBUTION-DOTPLOT-HISTODOT), you'll notice
...
Figure \@ref(fig:FIG-DISTRIBUTION-DOTPLOT-CENTER):
...
(Figure \@ref(fig:FIG-DISTRIBUTION-DOTPLOT-MULTI-BOXPLOT)):
...
shown in Figure \@ref(fig:FIG-DISTRIBUTION-DOTPLOT-MULTI-SIDE). This
...
scale_x\$\$\_\$\$continuous() to show *x* tick labels as text
...
Use stat_density2d(). Thismakes a 2D kernel density estimate
from the data. First we'll plot the density contour along with the
data points (Figure \@ref(fig:FIG-DISTRIBUTION-DENSITY2D), left):
...
(Figure \@ref(fig:FIG-DISTRIBUTION-DENSITY2D), right):
...
one-dimensional density estimate generated by stat_density(),
...
tiles, as shown in Figure \@ref(fig:FIG-DISTRIBUTION-DENSITY2D-TILE):
...
(Figure \@ref(fig:FIG-DISTRIBUTION-DENSITY2D-BANDWIDTH)), we'll use
...
The relationship between stat_density2d() and
stat_bin2d() is the same as the relationship between their
...
stat_density2d() passes options to kde2d(); see ?kde2d for
