...
```{r FIG-DISTRIBUTION-DENSITY-BASIC, ref.label=c("FIG-DISTRIBUTION-DENSITY-BASIC-1", "FIG-DISTRIBUTION-DENSITY-BASIC-2"), echo=FALSE, fig.show="hold", fig.cap="A kernel density estimate curve with `geom_density()` (left); With `geom_line()` (right)", fig.width=4, fig.height=4}
```
...
```{r FIG-DISTRIBUTION-DENSITY-WIDTH, fig.show="hold", fig.cap="Density curve with wider x limits and a semitransparent fill (left); In two parts, with `geom_density()` and `geom_line()` (right)", fig.width=4, fig.height=4}
ggplot(faithful, aes(x = waiting)) +
...
```{r FIG-DISTRIBUTION-DENSITY2D-TILE, fig.show="hold", fig.cap="With `..density..` mapped to fill (left); With points, and ..density.. mapped to alpha (right)", fig.width=5, fig.height=4}
