...
### Solution {#_solution_44}

Use geom\$\$\_\$\$histogram() andmap a continuous variable to x
([figure\_title](#FIG_DISTRIBUTION_HIST_BASIC)):
...
or a single vector of data. For this example weRSQUOll use the faithful
...
which is the length of time to the next eruption. WeRSQUOll only use the
waiting column in this example:

faithful
eruptions waiting 3.600 79 1.800 54 3.333 74 ...
If you just want to get a quick look at some data that isnRSQUOt in a
...
difficult to see which bar corresponds to which value, so weRSQUOll also
change the colors, as shown in
[figure\_title](#FIG_DISTRIBUTION_HIST_WIDTH).
...
[figure\_title](#FIG_DISTRIBUTION_HIST_QUANTIZE), weRSQUOll use a bin
width of 8. In the version on the left, weRSQUOll use the origin
parameter to put boundaries at 31, 39, 47, etc., while in the version on
the right, weRSQUOll shift it over by 4, putting boundaries at 35, 43,
51, etc.:
...
When visualizing your data, itRSQUOs a good idea to experiment with
...
### Solution {#_solution_45}

Use geom\$\$\_\$\$histogram() anduse facets for each group, as shown in
[figure\_title](#FIG_DISTRIBUTION_MULTI_HISTOGRAM_FACET):
...
and 1, and thereRSQUOs no label indicating that those values are for
smoke. To change the labels, we need to change the names of the factor
levels. First weRSQUOll take a look at the factor levels, then weRSQUOll
...
([figure\_title](#FIG_DISTRIBUTION_MULTI_HISTOGRAM_FACET), right).
...
([figure\_title](#FIG_DISTRIBUTION_MULTI_HISTOGRAM_FACET_SCALESFREE),
left):
...
[figure\_title](#FIG_DISTRIBUTION_MULTI_HISTOGRAM_FILL). The grouping
...
### Solution {#_solution_46}

Use geom\$\$\_\$\$density()and map a continuous variable to x
([figure\_title](#FIG_DISTRIBUTION_DENSITY_BASIC)):

```{r}
ggplot(faithful, aes(x=waiting)) + geom_density()
```

If you donRSQUOt like the lines along the side and bottom,you can use
geom\$\$\_\$\$line(stat="density") (see
[figure\_title](#FIG_DISTRIBUTION_DENSITY_BASIC), right):
...
at data that isnRSQUOt in a data frame, you can get the same result by
...
value of 1. [figure\_title](#FIG_DISTRIBUTION_DENSITY_ADJUST) shows what
...
([figure\_title](#FIG_DISTRIBUTION_DENSITY_WIDTH)). WeRSQUOll also add
...
weRSQUOll add geom\$\$\_\$\$histogram() first, and then layer
geom\$\$\_\$\$density() on top
([figure\_title](#FIG_DISTRIBUTION_DENSITY_HIST)):
...
itRSQUOs stored as a numeric vector, ggplot() doesnRSQUOt know that it
...
shown in [figure\_title](#FIG_DISTRIBUTION_MULTI_DENSITY_FACET). We can
align the facets vertically or horizontally. Here weRSQUOll align them
vertically so that itRSQUOs easy to compare the two distributions:
...
Now when we plot it again, it shows the new labels
([figure\_title](#FIG_DISTRIBUTION_MULTI_DENSITY_FACET), right):
...
the density curves. In this example, weRSQUOll also make the histogram
bars a little less prominent by changing the colors
([figure\_title](#FIG_DISTRIBUTION_MULTI_DENSITY_HIST)):
...
### Solution {#_solution_48}

Use geom\$\$\_\$\$freqpoly()
([figure\_title](#FIG_DISTRIBUTION_FREQPOLY)):
...
polygon ([figure\_title](#FIG_DISTRIBUTION_FREQPOLY), right):
...
low age lwt race smoke ptl ht ui ftv bwt 0 19 182 2 0 0 0 1 0 2523 0 33
155 3 0 0 0 0 3 2551 0 20 105 1 1 0 0 0 1 2557 ...
In [figure\_title](#FIG_DISTRIBUTION_BOXPLOT_BASIC), the data is divided
into groups by race, and we visualize the distributions of bwt for each
group. The value of race is 1, 2, or 3, but since itRSQUOs stored as a
numeric vector, ggplot() doesnRSQUOt know how to use it as a grouping
...
A box plot consists of a box and LDQUOwhiskers.RDQUO The box goes from
the 25th percentile to the 75th percentile of the data, also known as
the *inter-quartile range* (IQR). ThereRSQUOs a line indicating the
...
[figure\_title](#FIG_DISTRIBUTION_BOXPLOT_DIAGRAM) shows the
...
([figure\_title](#FIG_DISTRIBUTION_BOXPLOT_WIDTH_POINT), left):
...
([figure\_title](#FIG_DISTRIBUTION_BOXPLOT_WIDTH_POINT), right):
...
arbitrary value for x; otherwise, ggplot() wonRSQUOt know what *x*
coordinate to use for the box plot. In this case, weRSQUOll set it to 1
and remove the x-axis tick markers and label
([figure\_title](#FIG_DISTRIBUTION_BOXPLOT_SINGLE)):
...
### Solution {#_solution_50}

Use geom\$\$\_\$\$boxplot() and set notch=TRUE
([figure\_title](#FIG_DISTRIBUTION_BOXPLOT_NOTCH)):
...
With this particular data set, youRSQUOll see the following message:
...
itRSQUOs by such a small amount that you canRSQUOt see it in the final
output. ThereRSQUOs nothing inherently wrong with a notch going outside
...
### Solution {#_solution_51}

Use stat\$\$\_\$\$summary(). Themean is often shown with a diamond, so
weRSQUOll use shape 23 with a white fill. WeRSQUOll also make the
diamond slightly larger by setting size=3
([figure\_title](#FIG_DISTRIBUTION_BOXPLOT_MEAN)):
...
### Solution {#_solution_52}

Use geom\$\$\_\$\$violin()
([figure\_title](#FIG_DISTRIBUTION_VIOLIN_BASIC)):
...
a violin plot, itRSQUOs easier to compare several distributions since
theyRSQUOre placed side by side.
...
[figure\_title](#FIG_DISTRIBUTION_VIOLIN_BOXPLOT). Additionally, thebox
...
ends of the violins are at the extremes of the data. ItRSQUOs possible
to keep the tails, by setting trim=FALSE
([figure\_title](#FIG_DISTRIBUTION_VIOLIN_TAIL)):
...
group ([figure\_title](#FIG_DISTRIBUTION_VIOLIN_SCALECOUNT)). In this
...
([figure\_title](#FIG_DISTRIBUTION_VIOLIN_ADJUST)):
...
### Solution {#_solution_53}

Use geom\$\$\_\$\$dotplot(). Forthis example
([figure\_title](#FIG_DISTRIBUTION_DOTPLOT_BASIC)), weRSQUOll use a
...
ItRSQUOs different from the Cleveland dot plots shown in
...
y-axis tick marks that arenRSQUOt meaningful. The y-axis labels can be
removed by using scale\$\$\_\$\$y\$\$\_\$\$continuous(). In this
example, weRSQUOll also use geom\$\$\_\$\$rug() to show exactly where
each data point is
([figure\_title](#FIG_DISTRIBUTION_DOTPLOT_NO_Y_RUG)):
...
You may notice that the stacks arenRSQUOt regularly spaced in the
...
[figure\_title](#FIG_DISTRIBUTION_DOTPLOT_HISTODOT), youRSQUOll notice
that the stacks *arenRSQUOt* centered above the data:
...
[figure\_title](#FIG_DISTRIBUTION_DOTPLOT_CENTER):
...
Leland Wilkinson, LDQUODot Plots,RDQUO *The American Statistician* 53
(1999): 276–281,
...
### Solution {#_solution_54}

To compare multiple groups, itRSQUOs possible to stack the dots along
the y-axis, and group them along the x-axis, by setting binaxis="y". For
this example, weRSQUOll use the heightweight data set
([figure\_title](#FIG_DISTRIBUTION_DOTPLOT_MULTI)):
...
([figure\_title](#FIG_DISTRIBUTION_DOTPLOT_MULTI_BOXPLOT)):
...
ItRSQUOs also possible to show the dot plots next to the box plots, as
shown in [figure\_title](#FIG_DISTRIBUTION_DOTPLOT_MULTI_SIDE). This
...
### Solution {#_solution_55}

Use stat\$\$\_\$\$density2d(). Thismakes a 2D kernel density estimate
from the data. First weRSQUOll plot the density contour along with the
data points ([figure\_title](#FIG_DISTRIBUTION_DENSITY2D), left):
...
ItRSQUOs also possible to map the *height* of the density curve to the
color of the contour lines, by using ..level..
([figure\_title](#FIG_DISTRIBUTION_DENSITY2D), right):
...
to use contour lines, but itRSQUOs also possible to use tiles and map
the density estimate to the fill color, or to the transparency of the
tiles, as shown in [figure\_title](#FIG_DISTRIBUTION_DENSITY2D_TILE):
...
> (although it doesnRSQUOt matter in this particular case).
...
([figure\_title](#FIG_DISTRIBUTION_DENSITY2D_BANDWIDTH)), weRSQUOll use
