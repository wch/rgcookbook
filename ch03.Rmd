...
visualization. They're typically used to display numeric values (on
...
of items. A bar graph generally wouldn't be as good for showing
prices over time, where time is a continuous variable—though it can be
done, as we'll see in this chapter.

There's an important distinction you should be aware of when making
...
both of them. In this chapter I'll discuss this more, and present
...
([figure\_title](#FIG-BAR-GRAPH-BASIC-BAR)):
...
in [figure\_title](#FIG-BAR-GRAPH-BASIC-BAR-CONT). You can convert the
...
[figure\_title](#FIG-BAR-GRAPH-BASIC-BAR-SINGLE-FILL), we use a light
...
In this example we'll use the cabbage\$\$\_\$\$exp data set, which
...
We'll map Date to the *x* position and map Cultivar to the fill
color ([figure\_title](#FIG-BAR-GRAPH-GROUPED-BAR)):
...
and one continuous variable on the y-axis. Sometimes you'll want to
...
"dodge" each other horizontally; if you don't, you'll
...
[figure\_title](#FIG-BAR-GRAPH-GROUPED-BAR-OUTLINE) we'll use the
...
probably what you'll want to use.
...
data frame, we get [figure\_title](#FIG-BAR-GRAPH-GROUPED-BAR-MISSING):
...
([figure\_title](#FIG-BAR-GRAPH-COUNT)):
...
in [figure\_title](#FIG-BAR-GRAPH-COUNT-CONTINUOUS):
...
([figure\_title](#FIG-BAR-GRAPH-FILL)):
...
scale\$\$\_\$\$fill\$\$\_\$\$manual(). With this example, we'll use
the latter, and we'll set the outline color of the bars to black,
with colour="black" ([figure\_title](#FIG-BAR-GRAPH-FILL-MANUAL)). Note
...
color, as in [figure\_title](#FIG-BAR-GRAPH-COLOR-NEG). Notice that we
...
[figure\_title](#FIG-BAR-GRAPH-COLOR-NEG2). We'll also add a thin
...
values make the bars narrower ([figure\_title](#FIG-BAR-WIDTH)).
...
([figure\_title](#FIG-BAR-WIDTH-DODGE)).
...
[figure\_title](#FIG-BAR-WIDTH-DODGE-EXPLANATION). The two graphs both
...
[figure\_title](#FIG-BAR-GRAPH-STACKED-BAR):
...
[figure\_title](#FIG-BAR-GRAPH-STACKED-BAR-REVLEVELS), you can reverse
...
[figure\_title](#FIG-BAR-GRAPH-STACKED-BAR-REVSTACK), specify
...
([figure\_title](#FIG-BAR-GRAPH-STACKED-BAR-COLORS)):
...
[figure\_title](#FIG-BAR-GRAPH-PROP-STACKED-BAR):
...
([figure\_title](#FIG-BAR-GRAPH-PROP-STACKED-BAR-FINAL)):
...
shown in [figure\_title](#FIG-BAR-GRAPH-LABEL):
...
In [figure\_title](#FIG-BAR-GRAPH-LABEL), the *y* coordinates of the
...
using 3 ([figure\_title](#FIG-BAR-LABEL-GROUPED)):
...
The result is shown in [figure\_title](#FIG-BAR-LABEL-STACKED).
...
([figure\_title](#FIG-BAR-LABEL-STACKED-MIDDLE)), there must be an
...
([figure\_title](#FIG-BAR-LABEL-STACKED-FINAL)), we'll change the
...
[figure\_title](#FIG-BAR-GRAPH-DOTPLOT)) is to use
geom\$\$\_\$\$point():
...
In [figure\_title](#FIG-BAR-GRAPH-DOTPLOT) the names are sorted
...
dashed lines ([figure\_title](#FIG-BAR-GRAPH-DOTPLOT-ORDERED)):
...
[figure\_title](#FIG-BAR-GRAPH-DOTPLOT-ORDERED-SWAP). We'll also
...
To make the graph ([figure\_title](#FIG-BAR-GRAPH-DOTPLOT-ORDERED2)),
...
[figure\_title](#FIG-BAR-GRAPH-DOTPLOT-ORDERED2-FACET). The order in
which the facets are displayed is different from the sorting order in
[figure\_title](#FIG-BAR-GRAPH-DOTPLOT-ORDERED2); to change the display
