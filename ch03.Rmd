...
(Figure \@ref(fig:FIG-BAR-GRAPH-BASIC-BAR)):
...
in Figure \@ref(fig:FIG-BAR-GRAPH-BASIC-BAR-CONT). You can convert the
...
Map a variable to fill, anduse geom_bar(position="dodge").

In this example we'll use the cabbage_exp data set, which
...
color (Figure \@ref(fig:FIG-BAR-GRAPH-GROUPED-BAR)):
...
To set the colors, you can use scale_fill\$\$\_\$\$brewer() or
scale_fill\$\$\_\$\$manual(). In
Figure \@ref(fig:FIG-BAR-GRAPH-GROUPED-BAR-OUTLINE) we'll use the
...
data frame, we get Figure \@ref(fig:FIG-BAR-GRAPH-GROUPED-BAR-MISSING):
