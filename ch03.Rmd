...
If, instead of having `ggplot()` count up the number of rows in each group, you have a column in your data frame representing the y values, see Recipe \@ref(RECIPE-BAR-GRAPH-BASIC-BAR).
...
For more about using the `reorder()` function to reorder the levels of a factor based on the values of another variable, see Recipe \@ref(RECIPE-DATAPREP-FACTOR-REORDER-VALUE).
...
For more on transforming data by groups, see Recipe \@ref(RECIPE-DATAPREP-TRANSFORM-GROUP).
