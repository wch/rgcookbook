...
```{r, fig.cap="Left: bar graph of counts on a continuous axis"}
...
If, instead of having `ggplot()` count up the number of rows in each group, you have a column in your data frame representing the y values, see [section\_title](#RECIPE_BAR_GRAPH_BASIC_BAR).

You could also get the same graphical output by calculating the counts before sending the data to `ggplot()`. See [???](#RECIPE_DATAPREP_SUMMARIZE) for more on summarizing data.
...
For more about using the `reorder()` function to reorder the levels of a factor based on the values of another variable, see [???](#RECIPE_DATAPREP_FACTOR_REORDER_VALUE).
...
```{r FIG-BAR-GRAPH-COLOR-NEG, fig.cap="Different colors for positive and negative values"}
...
```{r FIG-BAR-GRAPH-COLOR-NEG2, fig.cap="Graph with customized colors and no legend"}
ggplot(csub, aes(x = Year, y = Anomaly10y, fill = pos)) +
...
To change the colors used, see Recipes [???](#RECIPE_COLORS_PALETTE_DISCRETE) and [???](#RECIPE_COLORS_PALETTE_DISCRETE_MANUAL).
...
If you make the entire graph wider or narrower, the bar dimensions will scale proportionally. To see how this works, you can just resize the window in which the graphs appear. For information about controlling this when writing to a file, see [???](#CHAPTER_OUTPUT).
...
It's also possible to modify the column of the data frame so that the factor levels are in a different order (see [???](#RECIPE_DATAPREP_FACTOR_REORDER)). Do this with care, since the modified data could change the results of other analyses.
...
To reorder the levels of a factor based on the values of another variable, see [???](#RECIPE_DATAPREP_FACTOR_REORDER_VALUE). To manually change the order of factor levels, see [???](#RECIPE_DATAPREP_FACTOR_REORDER).
...
First, scale the data to 100% within each stack. This can be done by using `group_by()` together with `mutate()` from the dplyr package. Then plot the resulting data, as shown in Figure \@ref(fig:FIG-BAR-GRAPH-PROP-STACKED-BAR):
...
This is what `cabbage_exp` looks like, and what modifcation does to it.
...
You may have noticed that `cabbage_exp` and `ce` print out differently. This is because `cabbage_exp` is a regular data frame, while `ce` is a *tibble*, which is a data frame with some extra properties. The dplyr package creates tibbles. For more information, see [TODO: tibble reference]().

Now that we've computed the percentages for `ce`, making the graph is the same as with a regular stacked bar graph.

As with regular stacked bar graphs, it makes sense to reverse change the color palette and add an outline. This is shown in (Figure \@ref(fig:FIG-BAR-GRAPH-PROP-STACKED-BAR-FINAL)):
...
For more on transforming data by groups, see [???](#RECIPE_DATAPREP_TRANSFORM_GROUP).
...
Notice that when the labels are placed atop the bars, they may be clipped. To remedy this, see [???](#RECIPE_AXES_RANGE).
...
When using labels, changes to the stacking order are best done by modifying the order of levels in the factor (see [???](#RECIPE_DATAPREP_FACTOR_REORDER)) before taking the cumulative sum. The other method of changing stacking order, by specifying breaks in a scale, won't work properly, because the order of the cumulative sum won't be the same as the stacking order.
...
To control the appearance of the text, see [???](#RECIPE_APPEARANCE_TEXT_APPEARANCE).
...
```{r FIG-BAR-GRAPH-DOTPLOT, fig.cap="Basic dot plot"}
library(gcookbook) # For the data set
...
```{r FIG-BAR-GRAPH-DOTPLOT-ORDERED, fig.cap="Dot plot, ordered by batting average"}
...
```{r FIG-BAR-GRAPH-DOTPLOT-ORDERED-SWAP, fig.cap="Dot plot with names on x-axis and values on y-axis"}
...
```{r FIG-BAR-GRAPH-DOTPLOT-ORDERED2, fig.cap="Grouped by league, with lines that stop at the point"}
...
```{r FIG-BAR-GRAPH-DOTPLOT-ORDERED2-FACET, fig.cap="Faceted by league"}
ggplot(tophit, aes(x = avg, y = name)) +
    geom_segment(aes(yend = name), xend = 0, colour = "grey50") +
    geom_point(size = 3, aes(colour = lg)) +
...
For more on changing the order of factor levels, see [???](#RECIPE_DATAPREP_FACTOR_REORDER). Also see [???](#RECIPE_DATAPREP_FACTOR_REORDER_VALUE) for details on changing the order of factor levels based on some other values.

For more on moving the legend, see [???](#RECIPE_LEGEND_POSITION). To hide grid lines, see [???](#RECIPE_APPEARANCE_HIDE_GRIDLINES).
