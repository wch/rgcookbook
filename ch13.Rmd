...
Use stat\$\$\_\$\$function(). It's also necessary to give ggplot() a
...
geom\$\$\_\$\$raster() (Figure \@ref(fig:FIG-MISCGRAPH-HEATMAP)). Simply
...
Use geom\$\$\_\$\$segment(). For this example, we'll use the isabel
data set:
...
ggplot2 has a stat\$\$\_\$\$qq() function, but it doesn't provide an
...
Use stat\$\$\_\$\$ecdf() (Figure \@ref(fig:FIG-MISCGRAPH-ECDF)):
...
geom\$\$\_\$\$path() (which can't have a fill). By default, the
...
because unlike most geoms, geom\$\$\_\$\$map() doesn't automatically
