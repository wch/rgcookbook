...
```{r FIG-BAR-WIDTH-DODGE, echo=FALSE, fig.show="hold", fig.cap="Bar graph with narrow grouped bars (left); With space between the bars (right)"}
ggplot(cabbage_exp, aes(x = Date, y = Weight, fill = Cultivar)) +
    geom_col(width = 0.5, position = "dodge")
ggplot(cabbage_exp, aes(x = Date, y = Weight, fill = Cultivar)) +
    geom_col(width = 0.5, position = position_dodge(0.7))
...
```{r FIG-BAR-GRAPH-LABEL, fig.show="hold", fig.cap="Labels under the tops of bars (left); Labels above bars (right)", fig.width=3.5, fig.height=3.5}
library(gcookbook) # For the data set
...
```{r FIG-LINE-GRAPH-MULTI-LINE-CONSTANT-AES, fig.show="hold", fig.cap="Line graph with constant size and color (left); With `supp` mapped to `colour`, and with points added (right)"}
...
```{r FIG-SCATTER-BASIC-SHAPE-SIZE, echo=FALSE, fig.show="hold", fig.cap="Scatter plot with hollow circles (shape 21, left); With smaller points (right)", fig.width=3.5, fig.height=3.5}
...
hw <-
  heightweight %>% 
  dplyr::mutate(weightgroup = ifelse(weightLb < 100, "< 100", ">= 100"))
...
heightweight %>% 
  dplyr::select(sex, ageYear, heightIn, weightLb)
```
...
# We will save a base plot called `hw_sp` (for heighweight scatter plot)
hw_sp <- 
  ggplot(heightweight, aes(x = ageYear, y = heightIn)) + 
    geom_point() +
    geom_line(data = pred)
...
countries %>% 
  dplyr::filter(Year == 2009 & healthexp > 2000)
```
...
cdat <-   
  countries %>% 
  dplyr::filter(Year == 2009, healthexp > 2000) %>% 
  dplyr::mutate(plotname = as.character(Name))
```
...
cdat <-
  countries %>% 
  dplyr::filter(Year == 2009, Name %in% countrylist)
...
library(tidyverse) # Load the tidyverse package to transform HairEyeColor from tables into long format (tibble)
...
# Creating the base balloon plot `hec_sp` (for HairEyeColor scatter plot)
hec_sp <- 
...
h <- 
  ggplot(faithful, aes(x = waiting))  # Save a base ggplot object
...
birthwt1 <- 
  birthwt %>% 
  dplyr::mutate(smoke = as.factor(smoke)) %>% # Convert smoke to a factor
  dplyr::mutate(smoke = fct_recode(smoke, 'No Smoke' = "0", 'Smoke' = "1"))
...
```{r FIG-DISTRIBUTION-MULTI-HISTOGRAM-FACET-SCALESFREE, ref.label=c("FIG-DISTRIBUTION-MULTI-HISTOGRAM-FACET-SCALESFREE-1", "FIG-DISTRIBUTION-MULTI-HISTOGRAM-FACET-SCALESFREE-2"), echo=FALSE, fig.show="hold", fig.cap='Histograms with the default fixed scales (left); With scales = "free" (right)', fig.width=3.5, fig.height=3.5}
```

Another approach is to map the grouping variable to `fill`, as shown in Figure \@ref(fig:FIG-DISTRIBUTION-MULTI-HISTOGRAM-FILL). The grouping variable must be a factor or a character vector. In the `birthwt` data set, the desired grouping variable, `smoke`, is stored as a number, so we’ll use the `birthwt1` data set we created above, in which smoke is a factor:
...
(ref:cap-FIG-DISTRIBUTION-DENSITY-WIDTH) Density curve with wider x limits
and a semitransparent fill (left); In two parts, with `geom_density()` and
`geom_line()` (right)
...
```{r FIG-DISTRIBUTION-MULTI-DENSITY-FACET, ref.label=c("FIG-DISTRIBUTION-MULTI-DENSITY-FACET-1", "FIG-DISTRIBUTION-MULTI-DENSITY-FACET-2", "FIG-DISTRIBUTION-MULTI-DENSITY-FACET-3"), echo=FALSE, fig.show="hold", fig.cap="Density curves with facets (left); With different facet labels (right)", fig.width=3.5, fig.height=3.5}
```
...
```{r FIG-DISTRIBUTION-FREQPOLY, ref.label=c("FIG-DISTRIBUTION-FREQPOLY-1", "FIG-DISTRIBUTION-FREQPOLY-2"), echo=FALSE, fig.show="hold", fig.cap="A frequency polygon (left); With wider bins (right)", fig.width=3.5, fig.height=3.5}
```
...
p <- 
  ggplot(heightwaes(x = sex, y = heightIn))
...
```{r FIG-DISTRIBUTION-VIOLIN-ADJUST, fig.show="hold", fig.cap="Violin plot with more smoothing (left); With less smoothing (right)", fig.width=3.5, fig.height=3.5}
...
```{r FIG-DISTRIBUTION-DENSITY2D, ref.label=c("FIG-DISTRIBUTION-DENSITY2D-1", "FIG-DISTRIBUTION-DENSITY2D-2"), echo=FALSE, fig.show="hold", fig.cap="Points and density contour (left); With ..level.. mapped to color (right)", fig.width=3.5, fig.height=3.5}
```
...
```{r FIG-DISTRIBUTION-DENSITY2D-TILE, fig.show="hold", fig.cap="With `..density..` mapped to fill (left); With points, and ..density.. mapped to alpha (right)", fig.width=3.5, fig.height=3.5}
...
```{r FIG-ANNOTATE-LINES, fig.show="hold", fig.cap="horizontal and vertical lines (left); angled line (right)", fig.width=3.5, fig.height=3.5}
library(gcookbook) # For the data set
...
```{r FIG-ANNOTATE-ERRORBAR-BASIC, fig.show="hold", fig.cap="error bars on a bar graph (left); on a line graph (right)", fig.width=3.5, fig.height=3.5}
library(gcookbook) # For the data set
...
```{r FIG-ANNOTATE-ERRORBAR-GROUP-BAR, fig.show="hold", fig.cap="error bars on a grouped bar graph without dodging width specified (left); with dodging width specified (right)", fig.width=3.5, fig.height=3.5}
...
```{r FIG-AXES-ORDER-MANUAL, ref.label=c("FIG-AXES-ORDER-MANUAL-1", "FIG-AXES-ORDER-MANUAL-2"), echo=FALSE, fig.cap="Box plot with manually specified items on the x-axis (left); With only two items (right)"}
```
...
```{r FIG-AXES-SCALE-EQUAL, ref.label=c("FIG-AXES-SCALE-EQUAL-1", "FIG-AXES-SCALE-EQUAL-2"), echo=FALSE, fig.show="hold", fig.cap="Scatter plot with equal scaling of axes (left); With tick marks at specified positions (right)"}
```
...
```{r FIG-AXES-AXIS-LABEL, fig.show="hold", fig.cap="Scatter plot with the default axis labels (left); Manually specified labels for the x- and y-axes (right)", fig.width=3.5, fig.height=3.5}
library(gcookbook) # For the data set
...
```{r FIG-AXES-LOG, fig.show="hold", fig.cap="Exponentially distributed data with linear-scaled axes (left); With logarithmic axes (right)"}
...
```{r FIG-AXES-LOG-BREAKS, ref.label=c("FIG-AXES-LOG-BREAKS-1", "FIG-AXES-LOG-BREAKS-2"), echo=FALSE, fig.show="hold", fig.cap="Scatter plot with log~10~ x- and y-axes, and with manually specified breaks (left); With exponents for the tick labels (right)"}
```
...
```{r FIG-AXES-POLAR-CONTINUOUS, ref.label=c("FIG-AXES-POLAR-CONTINUOUS-1", "FIG-AXES-POLAR-CONTINUOUS-2"), echo=FALSE, fig.show="hold", fig.cap="Polar plot with line (notice the data range of the radius) (left); With the radius representing a data range starting from zero (right)", fig.width=3.5, fig.height=3.5}
```
...
```{r FIG-AXES-POLAR-CONTINUOUS2, ref.label=c("FIG-AXES-POLAR-CONTINUOUS2-1", "FIG-AXES-POLAR-CONTINUOUS2-2"), echo=FALSE, fig.show="hold", fig.cap="Polar plot with theta representing x values from 0 to 12 (left); The gap is filled in by adding a dummy data point for month 0 (right)", fig.width=3.5, fig.height=3.5}
```
...
```{r FIG-APPEARANCE-TITLE-SUBTITLE, fig.show="hold", fig.cap="With a subtitle"}
p + ggtitle("Age and Height of Schoolchildren", "11.5 to 17.5 years old")
```
...
```{r FIG-APPEARANCE-TITLE-INSIDE, fig.show="hold", fig.cap="Title with ggtitle and a negative `vjust()` value (note the extra space above the plotting area) (left); With a text annotation at the top of the figure (right)"}
...
```{r FIG-LEGEND-POSITION, fig.cap="Legend on top (left); Legend inside of graphing area (right)"}
...
```{r FIG-LEGEND-POSITION-BACKGROUND, ref.label=c("FIG-LEGEND-POSITION-BACKGROUND-1", "FIG-LEGEND-POSITION-BACKGROUND-2"), echo=FALSE, fig.cap="Legend with opaque background and outline (left); With no background or outlines (right)"}
```
...
```{r FIG-FACET-LABEL-TEXT, fig.cap="Default facet labels (left); Modified facet labels (right)", fig.width=2.5}
...
ggplot(cabbage_exp, aes(x = Cultivar, y = Weight)) + geom_col() +
    facet_grid(. ~ Date) +
...
```{r FIG-COLORS-SETTING, fig.show="hold", fig.cap="Setting `fill` and `colour` (left); Setting `colour` for points (right)", message=FALSE}
library(MASS) # For the data set
...
```{r FIG-COLORS-MAPPING, echo=FALSE, fig.show="hold", fig.cap="Mapping a variable to `fill` (left); Mapping a variable to `colour` for points (right)"}
library(gcookbook) # For the data set
...
```{r FIG-COLORS-PALETTE-DISCRETE, fig.show="hold", fig.cap="Default palette (using hue) (left); A ColorBrewer palette (right)", fig.width=4.5}
library(gcookbook) # For the data set
...
```{r FIG-COLORS-PALETTE-LIGHTNESS, fig.show="hold", fig.cap="Points with default lightness (left); With lightness set to 45 (right)", fig.width=4.5}
# Basic scatter plot
...
```{r FIG-COLORS-PALETTE-GREY, fig.show="hold", fig.cap="Using the default grey palette (left); A different grey palette (right)", fig.width=4.5}
p + scale_fill_grey()
...
```{r FIG-COLORS-PALETTE-DISCRETE-MANUAL, fig.show="hold", fig.cap="Scatter plot with named colors (left); With slightly different RGB colors (right)", fig.width=4}
library(gcookbook) # For the data set
...
```{r FIG-MISCGRAPH-FUNCTION, ref.label=c("FIG-MISCGRAPH-FUNCTION-1", "FIG-MISCGRAPH-FUNCTION-2"), echo=FALSE, fig.show="hold", fig.cap="The normal distribution (left); The *t*-distribution with `df=2` (right)"}
````
...
```{r FIG-MISCGRAPH-3D-SCATTER3, eval=FALSE, fig.show="hold", fig.cap="3D scatter plot with axis ticks and labels repositioned (left); From a different point of view (right)"}
...
```{r FIG-MISCGRAPH-VECTORFIELD-AES, ref.label=c("FIG-MISCGRAPH-VECTORFIELD-AES-1", "FIG-MISCGRAPH-VECTORFIELD-AES-2"), echo=FALSE, fig.show="hold", fig.cap="Vector field with speed mapped to alpha (left); With speed mapped to colour (right)", fig.width=6, fig.height=5}
...
```{r FIG-MISCGRAPH-MAP-RESOLUTION, fig.show="hold", fig.cap="New Zealand data taken from `world` map (left); Data from `nz` map (right)"}
