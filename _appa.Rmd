...
data sets. See Recipes Recipe \@ref(RECIPE-DATAPREP-WIDE-TO-LONG) and
Recipe \@ref(RECIPE-DATAPREP-LONG-TO-WIDE) for information about converting
...
draw bars with geom_bar() don't worry about the other
...
change geom_bar() to geom_line(). We'll also map
...
drawn on the graph. We'll add geom_point() to draw points,
...
geom_point() to it. This has the same effect as the preceding
code:
...
mapping for this particular geom, geom_point(). If we added
...
geom_histogram(), automatically do this for you, but sometimes
...
explored in Chapter \@ref(CHAPTER-APPEARANCE).
