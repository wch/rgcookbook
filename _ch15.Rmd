...
Recipe \@ref(RECIPE-DATAPREP-SUBSET) for more on getting a subset
of a data frame.
...
legends. See Recipes Recipe \@ref(RECIPE-AXIS-ORDER) and
Recipe \@ref(RECIPE-LEGEND-ORDER) for more information.
...
axes and legends. See Recipes Recipe \@ref(RECIPE-AXIS-ORDER) and
Recipe \@ref(RECIPE-LEGEND-ORDER) for more information.
...
rename, see Recipe \@ref(RECIPE-DATAPREP-CHARACTER-RENAME).
...
rename, see Recipe \@ref(RECIPE-DATAPREP-FACTOR-RENAME).
...
Recipe \@ref(RECIPE-DATAPREP-FACTOR-RENAME).
...
See Recipe \@ref(RECIPE-DATAPREP-RECODE-CONTINUOUS) for recoding
...
Recipe \@ref(RECIPE-DATAPREP-RECODE-CATEGORICAL).

Transforming Variables {#RECIPE-DATAPREP-TRANSFORM}
----------------------
...
See Recipe \@ref(RECIPE-DATAPREP-TRANSFORM-GROUP) for how to
perform group-wise transformations on data.

Transforming Variables by Group {#RECIPE-DATAPREP-TRANSFORM-GROUP}
-------------------------------
...
Recipe \@ref(RECIPE-DATAPREP-SUMMARIZE).
...
Recipe \@ref(RECIPE-DATAPREP-SUMMARIZE-SE).

See Recipe \@ref(RECIPE-DISTRIBUTION-BOXPLOT-MEAN) for an example of using
stat_summary() to calculate means and overlay them on a graph.

To perform transformations on data by groups, see
Recipe \@ref(RECIPE-DATAPREP-TRANSFORM-GROUP).
...
See Recipe \@ref(RECIPE-ANNOTATE-ERROR-BAR) to use the values calculated here
...
See Recipe \@ref(RECIPE-DATAPREP-LONG-TO-WIDE) to do conversions in
...
See Recipe \@ref(RECIPE-DATAPREP-WIDE-TO-LONG) to do conversions in
