...
don't fit into nice, tidy categories. This chapter shows how to make
...
(Figure \@ref(fig:FIG-MISCGRAPH-CORRMATRIX-BASIC)), we'll use the
...
(Figure \@ref(fig:FIG-MISCGRAPH-CORRMATRIX-SHADE)):
...
coefficient on each square in the matrix. In this example we'll make
a lighter palette so that the text is readable, and we'll remove the
color legend, since it's redundant. We'll also order the items
...
Figure \@ref(fig:FIG-MISCGRAPH-CORRMATRIX-SHADE-NUM):
...
menagerie of options, which can't all be illustrated here.
...
Use stat_function(). It's also necessary to give ggplot() a
dummy data frame so that it will get the proper *x* range. In this
example we'll use dnorm(), which gives the density of the normal
distribution (Figure \@ref(fig:FIG-MISCGRAPH-FUNCTION), left):
...
degrees of freedom (Figure \@ref(fig:FIG-MISCGRAPH-FUNCTION), right).
...
It's also possible to define your own functions. It should take an
*x* value for its first argument, and it should return a *y* value. In
this example, we'll define a sigmoid function
(Figure \@ref(fig:FIG-MISCGRAPH-FUNCTION-CUSTOM)):
...
Figure \@ref(fig:FIG-MISCGRAPH-FUNCTION-SHADE):
...
what to return, conditional on the value of x. But that won't work
here, since x is a vector with many values.

### Discussion

R has first-class functions, and we can write a function that returns a
*closure*-that is, we can program a function to program another
function.
...
Now we can call this function to create another function-one that is
effectively the same as the dnorm_limit() function used
earlier:
...
Figure \@ref(fig:FIG-MISCGRAPH-FUNCTION-SHADE), you may see that the
...
specified range. As in Recipe \@ref(RECIPE-MISCGRAPH-FUNCTION), we
can improve the approximation by increasing the number of
interpolatedvalues with stat_function(n=200).
...
(Figure \@ref(fig:FIG-MISCGRAPH-GRAPH-BASIC)):
...
data, and they're placed randomly. To make the output repeatable,
...
It's also possible to create a graph from a data frame. The first
two columns of the data frame are used, and each row specifies a
connection between two nodes. In the next example
(Figure \@ref(fig:FIG-MISCGRAPH-GRAPH-DIRECTED)), we'll use the
madmen2 data set, which has this structure. We'll also use the
...
doesn't matter for an undirected graph. This time we'll use a
circle layout (Figure \@ref(fig:FIG-MISCGRAPH-GRAPH-CIRCLE)):
...
vertex.label (Figure \@ref(fig:FIG-MISCGRAPH-GRAPH-LABEL)):
...
(Figure \@ref(fig:FIG-MISCGRAPH-GRAPH-LABEL-EDGE)):
...
Use geom_tile() or geom_raster() and map a continuous
variable to fill. We'll use the presidents data set, which is a time
...
3 24 1974 4
Now we can make the plot using geom_tile() or
geom_raster() (Figure \@ref(fig:FIG-MISCGRAPH-HEATMAP)). Simply
...
![A heat map-the grey squares represent NAs in the
data](figs/rgcb_1312.png)

> **Note**
>
> The results with geom_tile() and geom_raster()
> *should* look the same, but in practice they might appear different.
> See Recipe \@ref(RECIPE-DISTRIBUTION-DENSITY2D) for more information about
> this issue.

### Discussion

To better convey useful information, you may want to customize the
appearance of the heat map. With this example, we'll reverse the
y-axis so that it progresses from top to bottom, and we'll add tick
marks every four years along the x-axis, to correspond with each
presidential term. We'll also change the color scale using
scale_fill\$\$\_\$\$gradient2(), which lets you specify a
midpoint color and the two colors at the low and high ends
(Figure \@ref(fig:FIG-MISCGRAPH-HEATMAP-CUSTOM)):
...
Recipe \@ref(RECIPE-COLORS-PALETTE-CONTINUOUS).
...
We'll use the rgl package, which provides an interface to the OpenGL
graphics library for 3D graphics. To create a 3D scatter plot, as in
Figure \@ref(fig:FIG-MISCGRAPH-3D-SCATTER), use plot3d() and pass in a
...
it's often better to use a two-dimensional representation of the
data. That said, there are things that can help make a 3D scatter plot
easier to understand.

In Figure \@ref(fig:FIG-MISCGRAPH-3D-SCATTER2), we'll add vertical
...
It's possible to tweak the appearance of the background and the
axes. In Figure \@ref(fig:FIG-MISCGRAPH-3D-SCATTER3), we change the
...
Figure \@ref(fig:FIG-MISCGRAPH-3D-SCATTER-MODEL):
...
Figure \@ref(fig:FIG-MISCGRAPH-3D-SCATTER-MODEL2). We'll add each of
...
If you don't want to use ImageMagick to convert the output to a
...
data preprocessing. For this example, we'll first take a subset of
the countries data set from the year 2009. For simplicity, we'll
...
set the *row names*-right now there's a column called Name, but the
row names are those random numbers (we don't often use row names,
but for the hclust() function they're essential). Next, we'll
need to drop all the columns that aren't values used for clustering.
...
of GDP.This probably isn't what we want. To address this issue,
...
Finally, we're ready to make the dendrogram, as shown in
Figure \@ref(fig:FIG-MISCGRAPH-DENDROGRAM):
...
analysis process, and there may not be a single "right" way to
do it for your data.
...
Figure \@ref(fig:FIG-MISCGRAPH-DENDROGRAM-UNSCALED).)

![Dendrogram with unscaled data-notice the much larger Height values,
...
Use geom_segment(). For this example, we'll use the isabel
data set:
...
To draw the vectors (Figure \@ref(fig:FIG-MISCGRAPH-VECTOR)), we'll
use geom_segment(). Each segment has a starting point and an
ending point. We'll use the x and y values as the starting points
for each segment, then add a fraction of the vx and vy values to get the
end points for each segment. If we didn't scale down these values,
...
![First attempt at a vector field-the resolution of the data is too
...
the flow. To reduce the resolution of the data, we'll define a
function every_n() that keeps one out of every n values in the
...
Now that we've taken a subset of the data, we can plot it, with
arrowheads, as shown in
Figure \@ref(fig:FIG-MISCGRAPH-VECTORFIELD-ARROW):
...
alpha, or colour. Here, we'll map speed to alpha
(Figure \@ref(fig:FIG-MISCGRAPH-VECTORFIELD-AES), left):
...
Next, we'll map speed to colour. We'll also add a map of the
United States and zoom in on the area of interest, as shown in the graph
on the right in Figure \@ref(fig:FIG-MISCGRAPH-VECTORFIELD-AES),using
coord_cartesian() (without this, the entire USA will be
displayed):
...
Figure \@ref(fig:FIG-MISCGRAPH-VECTORFIELD-FACET). Because each facet is
...
See Recipe \@ref(RECIPE-AXES-RANGE) for more information about zooming in on
part of a graph.
...
qqline() (Figure \@ref(fig:FIG-MISCGRAPH-QQ)):
...
ggplot2 has a stat_qq() function, but it doesn't provide an
...
we'll use the USBAdmissions data set, which is a contingency table
with three dimensions. We'll first take a look at the data in a few
different ways:
...
(Figure \@ref(fig:FIG-MISCGRAPH-MOSAIC)), use mosaic() and pass it a
...
![Mosaic plot of UC-Berkeley admissions data-the area of each rectangle
...
Figure \@ref(fig:FIG-MISCGRAPH-MOSAIC2). This makes the admission status
...
Figure \@ref(fig:FIG-MISCGRAPH-MOSAIC3) and
Figure \@ref(fig:FIG-MISCGRAPH-MOSAIC4):
...
The example here illustrates a classic case of Simpson's paradox, in
...
In Figures Figure \@ref(fig:FIG-MISCGRAPH-MOSAIC2) and
Figure \@ref(fig:FIG-MISCGRAPH-MOSAIC3), you can see that within each
...
Figure \@ref(fig:FIG-MISCGRAPH-MOSAIC3), as in
Figure \@ref(fig:FIG-MISCGRAPH-MOSAIC2), it’s easy to compare male and
...
as in Figure \@ref(fig:FIG-MISCGRAPH-MOSAIC4), makes it difficult to
...
Graduate Admissions: Data from Berkeley," *Science* 187 (1975):
398–404.
...
(Figure \@ref(fig:FIG-MISCGRAPH-PIE)), we'll use the survey data set
...
visualization experts. If you're thinking of using a pie chart,
...
The map_data() functionreturns a data frame with the following
columns:
...
isn't a separate map, you can first look for the region name, like
so:
...
It's possible to get data for specific regions from a particular map
(Figure \@ref(fig:FIG-MISCGRAPH-MAP-REGIONS)):
...
Figure \@ref(fig:FIG-MISCGRAPH-MAP-RESOLUTION):
...
(Figure \@ref(fig:FIG-MISCGRAPH-CHOROPLETH)), mapping one of the columns
...
middle value, you can use scale_fill\$\$\_\$\$gradient2(), as
shown in Figure \@ref(fig:FIG-MISCGRAPH-CHOROPLETH-MIDPOINT):
...
data with the value data, is to use geom_map(). As of this
...
specified by mapping it to the map_id aesthetic. For example,
this code will have the same output as the first example
(Figure \@ref(fig:FIG-MISCGRAPH-CHOROPLETH)):
...
Notice that we also needed to use expand_limits(). This is
because unlike most geoms, geom_map() doesn't automatically
set the x and y limits; the use of expand_limits() makes it
...
Recipe \@ref(RECIPE-MISCGRAPH-VECTORFIELD).
...
Then add it to the map (Figure \@ref(fig:FIG-MISCGRAPH-MAP-BACKGROUND)).
In this example, we'll add it to one of the choropleths we created
in Recipe \@ref(RECIPE-MISCGRAPH-CHOROPLETH):
...
> **Note**
>
> There's a bug in R versions 2.15.2 and earlier, which may throw an
...
In some maps, it's important to include contextual information such
as the latitude and longitude. In others, this information is
unimportant and distracts from the information that's being
conveyed. In Figure \@ref(fig:FIG-MISCGRAPH-MAP-BACKGROUND), it's
...
position, and even if they can't, having the latitude and longitude
...
(Figure \@ref(fig:FIG-MISCGRAPH-MAP-SHAPEFILE)):
...
It's actually possible to pass the SpatialPolygonsDataFrame object
