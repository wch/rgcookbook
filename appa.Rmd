...
Most of the recipes in this book involve the ggplot2 package, written
byHadley Wickham. ggplot2 has only been around for a few years, but in
that short time it has attracted many users in the R community because
of its versatility, clear and consistent interface, and beautiful
output.

ggplot2 takes a different approach to graphics than other graphing
packages in R. It gets its name from Leland WilkinsonRSQUOs *grammar of
graphics*, which provides a formal, structured perspective on how to
describe data graphics.

Even though this book deals largely with ggplot2, I donRSQUOt mean to
say that itRSQUOs the be-all and end-all of graphics in R. For example,
I sometimes find it faster and easier to inspect and explore data with
RRSQUOs base graphics, especially when the data isnRSQUOt already
structured properly for use with ggplot2. There are some things that
ggplot2 canRSQUOt do, or canRSQUOt do as well as other graphing
...
and it provides beautiful, publication-ready results.

Another excellent package for general-purpose graphs is lattice,
byDeepyan Sarkar, which is an implementation of *trellis* graphics. It
is included as part of the base installation of R.
...
lines, heights of bars, and so on. A data visualization that didnRSQUOt
map the data to visual properties wouldnRSQUOt be a data visualization.
...
thatRSQUOs where our understanding remains.

In the grammar of graphics, this deep similarity is not just recognized,
but made central. In RRSQUOs base graphics functions, each mapping of
...
To illustrate, IRSQUOll show a graph made from the simpledat data set
...
before ([figure\_title](#app_figure_16-2)):
...
instead of bars, as shown in [figure\_title](#app_figure_16-3). To do
...
Now letRSQUOs take a look at the corresponding code and graphs with
ggplot2. With ggplot2, the structure of the data is always the same: it
requires a data frame in LDQUOlongRDQUO format, as opposed to the
LDQUOwideRDQUO format used previously. When the data is in long format,
...
To make the first grouped bar graph ([figure\_title](#app_figure_16-4)),
...
draw bars with geom\$\$\_\$\$bar() donRSQUOt worry about the other
details yet; weRSQUOll get to those later):
...
the grouping ([figure\_title](#app_figure_16-5)), we simply swap the
mapping specification, with x=Bval and fill=Aval. Unlike with base
graphics, we donRSQUOt have to change the data; we just change the
...
To change it to a line graph ([figure\_title](#app_figure_16-6)), we
change geom\$\$\_\$\$bar() to geom\$\$\_\$\$line(). WeRSQUOll also map
...
donRSQUOt worry about the other details yet:
...
Before we go any further, itRSQUOll be helpful to define some of the
...
HereRSQUOs an example of how a typical mapping works. You have *data*,
...
These arenRSQUOt the only kinds of data and aesthetic spaces possible.
...
> always correspond to how theyRSQUOre used elsewhere. Sometimes a
...
> integer, so itRSQUOs numeric (*continuous* to ggplot2) and discrete
...
WeRSQUOll walk through a simple example here. First, weRSQUOll make a
...
After weRSQUOve given ggplot() the data frame and the aesthetic
mappings, thereRSQUOs one more critical component: we need to tell it
what *geometric objects* to put there. At this point, ggplot2
doesnRSQUOt know if we want bars, lines, points, or something else to be
drawn on the graph. WeRSQUOll add geom\$\$\_\$\$point() to draw points,
...
If youRSQUOre going to reuse some of these components, you canstore them
...
This doesnRSQUOt alter the *default* aesthetic mappings that we defined
...
we wonRSQUOt use aes(); weRSQUOll just set the value of colour directly:
...
visual attributes. Here, weRSQUOll change the *x* scale so that it has a
larger range:
...
Notice that weRSQUOve used \$\$+\$\$ to join together the pieces. In
...
the next line. Otherwise, RRSQUOs parser wonRSQUOt know that thereRSQUOs
more stuff coming; itRSQUOll think youRSQUOve finished the expression
and evaluate it.

Printing {#_printing}
--------

In RRSQUOs base graphics, the graphing functions tell R to draw graphs
to the output device (the screen or a file). Ggplot2 is a little
different. The commands donRSQUOt directly draw to the output device.
Instead, the functions build plot *objects*, and the graphs arenRSQUOt
drawn until you use the print() function, as in print(*object*). You
might be thinking, LDQUOBut wait, I havenRSQUOt told R to print
anything, yet itRSQUOs made these graphs!RDQUO Well, thatRSQUOs not
...
a script or function. In scripts, commands arenRSQUOt automatically
...
printed because itRSQUOs the result of the function.
...
> IRSQUOve found it a bit confusing to use because it has a slightly
> different way of specifying certain graphing parameters. I think
> itRSQUOs simpler and easier to just use ggplot().
...
youRSQUOll want to do this yourself, using various stat\$\$\_\$\$*xx*
functions.

Themes {#_theme}
------

Some aspects of a graphRSQUOs appearance fall outside the scope of the
