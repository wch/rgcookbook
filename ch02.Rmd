...
```{r FIG-QUICK-SCATTER-BASE, small.mar=TRUE, fig.cap='Scatter plot with base graphics'}
...
With ggplot2, you can get a similar result using `geom_bar()` (Figure \@ref(fig:FIG-QUICK-BAR-GGPLOT)). To plot a bar graph of *values*, use `geom_bar(stat="identity")`. Notice the difference in the output when the *x* variable is continuous and when it is discrete:

```{r FIG-QUICK-BAR-GGPLOT, fig.show="hold", fig.cap="Left: bar graph of values using `geom_bar(stat=\"identity\")` with a continuous x variable; right: with x variable converted to a factor (notice that there is no entry for 6)"}
library(ggplot2)
...
ggplot2 can also be used to plot the *count* of the number of data rows in each category (Figure \@ref(fig:FIG-QUICK-BAR-GGPLOT-COUNT)). This is in fact the default way that ggplot2 creates bar graphs, and requires less typing than a bar graph of values. Once again, notice the difference between a continuous x-axis and a discrete one. For data like this, it makes more sense to convert the continuous x variable to a discrete one, with the `factor()` function.

```{r FIG-QUICK-BAR-GGPLOT-COUNT, fig.show="hold", fig.cap="Left: bar graph of counts using `geom_bar()` with a continuous x variable; right: with x variable converted to a factor"}
...
```{r FIG-QUICK-BOXPLOT-BASE, echo=FALSE, small.mar=TRUE, fig.show="hold", fig.cap="Left: box plot with base graphics; right: with multiple grouping variables", fig.width=10}
layout(t(c(1,2,2)))
plot(ToothGrowth$supp, ToothGrowth$len)
...
```{r FIG-QUICK-FUNCTION-GGPLOT, fig.cap="A function curve with ggplot2"}
library(ggplot2)
