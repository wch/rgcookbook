...
With ggplot2, you can get a similar result using qplot() with geom="line" (Figure \@ref(fig:FIG-QUICK-LINE-QPLOT)):
...
To make a bar graph of values (Figure \@ref(fig:FIG-QUICK-BAR-BASE)), use barplot() and pass it a vector of values for the height of each bar and (optionally) a vector of labels for each bar. If the vector has names for the elements, the names will automatically be used as labels:
...
With the ggplot2 package, you can get a similar result using qplot() (Figure \@ref(fig:FIG-QUICK-BAR-QPLOT)). To plot a bar graph of *values*, use geom="bar" and stat="identity". Notice the difference in the output when the *x* variable is continuous and when it is discrete:
...
qplot() can also be used to graph the *counts* in each category (Figure \@ref(fig:FIG-QUICK-BAR-QPLOT-COUNT)). This is in fact the default way that ggplot2 creates bar graphs, and requires less typing than a bar graph of values. Once again, notice the difference between a continuous x-axis and a discrete one.
...
(Figure \@ref(fig:FIG-QUICK-HIST-QPLOT)):
...
With the ggplot2 package, you can get a similar resultusing qplot() (Figure \@ref(fig:FIG-QUICK-BOXPLOT-QPLOT)), with geom="boxplot":
...
It's also possible to make box plots for multiple variables, by combining the variables with interaction(), as in Figure \@ref(fig:FIG-QUICK-BOXPLOT-QPLOT). In this case, the dose variable is numeric, so we must convert it to a factor to use it as a grouping variable:
