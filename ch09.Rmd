...
```{r FIG-APPEARANCE-TITLE, fig.show="hold", fig.cap="Scatter plot with a title added (left); With a `\\n` for a newline (right)"}
...
```{r FIG-APPEARANCE-TEXT-APPEARANCE-THEME, fig.show="hold", fig.cap='`axis.title.x` (left); `plot.title` (right)'}
library(gcookbook)  # Load gcookbook for the heightweight data set
...
```{r FIG-APPEARANCE-TEXT-APPEARANCE-GEOM, fig.show="hold", fig.cap='`annotate("text")` (left); `geom_text()` (right)'}
hw_plot +
...
```{r FIG-APPEARANCE-THEME, fig.show="hold", fig.cap="Scatter plot with `theme_grey()` (the default, top left); with `theme_bw()` (top right); with `theme_minimal()` (bottom left); with `theme_classic()` (bottom right)"}
library(gcookbook)  # Load gcookbook for the heightweight data set
...
```{r FIG-APPEARANCE-THEME-VOID, fig.show="hold", fig.cap="Scatter plot with `theme_void()`"}
hw_plot +
  theme_void()
```
