...
library(gcookbook) # Load the gcookbook package to access the worldpop data set
...
library(gcookbook) # Load the gcookbook package to access the ToothGrowth data set
...
ggplot(ToothGrowth, aes(x = dose, y = len, linetype = supp)) + 
  geom_line()
```

### Discussion

The `ToothGrowth` data has three columns, including the factor `supp`, which we mapped to `colour` and `linetype`:
...
ggplot(ToothGrowth, aes(x = factor(dose), y = len, colour = supp, group = supp)) + 
  geom_line()
```
...
```{r eval=FALSE}
ggplot(ToothGrowth, aes(x = factor(dose), y = len, colour = supp)) + geom_line()
...
ggplot(ToothGrowth, aes(x = dose, y = len, shape = supp)) + 
  geom_line() +
