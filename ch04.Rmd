...
variable is ordered (e.g., LDQUOsmallRDQUO, LDQUOmediumRDQUO,
LDQUOlargeRDQUO), but not when the variable is unordered (e.g.,
LDQUOcowRDQUO, LDQUOgooseRDQUO, LDQUOpigRDQUO). Most of the examples in
this chapter use a continuous *x* variable, but weRSQUOll see one
...
### Solution {#_solution_22}

Use ggplot() with geom\$\$\_\$\$line(), and specify what variables you
mapped to x and y ([figure\_title](#FIG_LINE_GRAPH_BASIC_LINE)):
...
([figure\_title](#FIG_LINE_GRAPH_BASIC_LINE_FACTOR)). When the *x*
...
### Solution {#_solution_23}

Add geom\$\$\_\$\$point() ([figure\_title](#FIG_LINE_GRAPH_POINT)):
...
[figure\_title](#FIG_LINE_GRAPH_BASIC_LINE_YLIM) with
[figure\_title](#FIG_LINE_GRAPH_POINT)).
...
([figure\_title](#FIG_LINE_GRAPH_POINTS_INTERVAL)):
...
being categorical, even when itRSQUOs stored as a number. In the example
...
([figure\_title](#FIG_LINE_GRAPH_MULTI_LINE_FACTOR)):
...
Notice the use of group=supp. Without this statement, ggplot() wonRSQUOt
...
[figure\_title](#FIG_LINE_GRAPH_MULTI_LINE_SAWTOOTH):
...
location, and ggplot() thinks theyRSQUOre all in one group. The data
...
arenRSQUOt mapped to an aesthetic), they should be used with group.

> **Note**
>
> When in doubt, if your line graph looks wrong, try explicitly
> specifying the grouping variable with group. ItRSQUOs common for
...
([figure\_title](#FIG_LINE_GRAPH_MULTI_LINE_SHAPE_FILL)):
...
([figure\_title](#FIG_LINE_GRAPH_MULTI_LINE_DODGE)). When doing so, you
...
[figure\_title](#FIG_BAR_GRAPH_BASIC_LINE_CUSTOMIZED)) by passing them
values in the call to geom\$\$\_\$\$line():
...
line looking different. The default colors arenRSQUOt the most
appealing, so you may want to use a different palette, as shown in
[figure\_title](#FIG_LINE_GRAPH_MULTI_LINE_PALETTE), by using
scale\$\$\_\$\$colour\$\$\_\$\$brewer() or
...
([figure\_title](#FIG_LINE_GRAPH_MULTI_LINE_CONSTANT_AES)). You may also
...
### Solution {#_solution_26}

In geom\$\$\_\$\$point(), setthe size, shape, colour, and/or fill
outside of aes() (the result is shown in
[figure\_title](#FIG_LINE_GRAPH_POINT_PROPERTIES)):
...
[figure\_title](#FIG_LINE_GRAPH_POINT_WHITEFILL):
...
scale\$\$\_\$\$colour\$\$\_\$\$brewer() or
scale\$\$\_\$\$colour\$\$\_\$\$manual(). To set a single constant shape
or size for all the points, as in
[figure\_title](#FIG_LINE_GRAPH_MULTI_LINE_POINT_PALETTE), specify shape
...
### Solution {#_solution_27}

Use geom\$\$\_\$\$area() toget a shaded area, as in
[figure\_title](#FIG_LINE_GRAPH_AREA):
...
example, weRSQUOll set it to "blue", and weRSQUOll also make it 80%
transparent by setting alpha to 0.2. This makes it possible to see the
grid lines through the area, as shown in
[figure\_title](#FIG_LINE_GRAPH_AREA_OUTLINE). WeRSQUOll also add an
...
geom\$\$\_\$\$line() on top, as shown in
[figure\_title](#FIG_LINE_GRAPH_AREA_TOPLINE):
...
### Solution {#_solution_28}

Use geom\$\$\_\$\$area() andmap a factor to fill
([figure\_title](#FIG_LINE_GRAPH_STACKED_AREA)):
...
([figure\_title](#FIG_LINE_GRAPH_STACKED_AREA_CUSTOMIZED)) reverses the
...
To reverse the stacking order, weRSQUOll put order=desc(AgeGroup) inside
of aes() ([figure\_title](#FIG_LINE_GRAPH_STACKED_AREA_REVSTACK)):
...
rid of it ([figure\_title](#FIG_LINE_GRAPH_STACKED_AREA_NOSIDES)), first
draw the stacked areas *without* an outline (by leaving colour as the
default NA value), and then add a geom\$\$\_\$\$line() on top:
...
### Solution {#_solution_29}

First, calculate the proportions. In this example, weRSQUOll useddply()
...
Once weRSQUOve calculated the proportions, plotting is the same as with
a regular stacked area graph
([figure\_title](#FIG_LINE_GRAPH_PROPORTIONAL_STACKED_AREA)):
...
### Discussion {#_discussion_25}

LetRSQUOs take a closer look at the data and how it was summarized:

uspopage
Year AgeGroup Thousands 1900 &lt;5 9181 1900 5-14 16966 1900 15-24 14951
1900 25-34 12161 1900 35-44 9273 1900 45-54 6437 1900 55-64 4026 1900
&gt;64 3099 1901 &lt;5 9336 1901 5-14 17158 ...
WeRSQUOll use ddply() to splitit into separate data frames for each
...
### Solution {#_solution_30}

Use geom\$\$\_\$\$ribbon() and map values to ymin and ymax.
...
Unc10y is the 95% confidence interval. WeRSQUOll set ymax and ymin to
Anomaly10y plus or minus Unc10y
([figure\_title](#FIG_LINE_GRAPH_REGION)):
...
### Discussion {#_discussion_26}

Notice that the geom\$\$\_\$\$ribbon() is before geom\$\$\_\$\$line(),
...
particular case that wouldnRSQUOt be a problem since the shaded region
...
([figure\_title](#FIG_LINE_GRAPH_REGION_LINES)):
