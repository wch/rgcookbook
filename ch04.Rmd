...
See [???](#RECIPE_AXES_RANGE) for more on controlling the range of the axes.
...
To change the appearance of the points, see [section\_title](#RECIPE_LINE_GRAPH_POINT_APPEARANCE).
...
```{r FIG-LINE-GRAPH-MULTI-LINE-COLOR-TYPE, fig.show="hold", fig.cap="Left: a variable mapped to colour; right: a variable mapped to linetype", fig.width=3.5, fig.height=3.5}
# Use dplyr to create the example data set
library(dplyr)
...
```{r FIG-LINE-GRAPH-MULTI-LINE-SHAPE-FILL, fig.show="hold", fig.cap="Left: line graph with different shapes; right: with different colors", fig.width=3.5, fig.height=3.5}
...
```{r FIG-LINE-GRAPH-MULTI-LINE-CONSTANT-AES, fig.show="hold", fig.cap="Left: line graph with constant size and color; right: with supp mapped to colour, and with points added", fig.width=3.5, fig.height=3.5}
...
See [???](#RECIPE_SCATTER_SHAPES) for more on using different shapes, and [???](#CHAPTER_COLORS) for more about colors.
...
```{r FIG-LINE-GRAPH-AREA, fig.cap="Graph with a shaded area"}
...
```{r FIG-LINE-GRAPH-AREA-OUTLINE, fig.cap="Graph with a semitransparent shaded area and an outline"}
...
```{r FIG-LINE-GRAPH-AREA-TOPLINE, fig.cap="Line graph with a line just on top, using geom_line()"}
ggplot(sunspotyear, aes(x = Year, y = Sunspots)) +
...
```{r FIG-LINE-GRAPH-STACKED-AREA, fig.cap="Stacked area graph"}
library(gcookbook) # For the data set
...
The sort of data that is plotted with a stacked area chart is often provided in a wide format, but ggplot requires data to be in long format. To convert it, see [???](#RECIPE_DATAPREP_WIDE_TO_LONG).
...
```{r FIG-LINE-GRAPH-STACKED-AREA-CUSTOMIZED, fig.cap="Reversed legend order, lines, and a different palette"}
...
```{r FIG-LINE-GRAPH-STACKED-AREA-NOSIDES, fig.cap="No lines on the left and right of the graph"}
...
See [???](#RECIPE_DATAPREP_WIDE_TO_LONG) for more on converting data from wide to long format.
...
First, calculate the proportions. In this example, we'll use the dplyr package to break `uspopage` into groups by `Year`, then calculate a new column, `Percent`. This value is the `Thousands` for each row, divided by the sum of `Thousands` for each `Year` group, multiplied by 100 to get a percent value:
...
Once we've calculated the proportions, plotting is the same as with a regular stacked area graph (Figure \@ref(fig:FIG-LINE-GRAPH-PROPORTIONAL-STACKED-AREA)):

```{r FIG-LINE-GRAPH-PROPORTIONAL-STACKED-AREA, fig.cap="A proportional stacked area graph"}
...
We used dplyr's `group_by()` function to group it into separate data frames for each value of Year, then used the `mutate()` function to each piece to calculate the `Percent` for each piece:
...
For more on summarizing data by groups, see [???](#RECIPE_DATAPREP_SUMMARIZE).
...
```{r FIG-LINE-GRAPH-REGION, fig.cap="A line graph with a shaded confidence region"}
library(gcookbook) # For the data set
...
```{r A line graph with a shaded confidence region, fig.cap="A line graph with dotted lines representing a confidence region"}
...
In the area graphs in [section\_title](#RECIPE_LINE_GRAPH_STACKED_AREA), the *y* range of the shaded area goes from 0 to y. Here, it goes from `ymin` to `ymax`.
