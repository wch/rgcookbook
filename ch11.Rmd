...
```{r FIG-FACET-FACET-GRID, fig.cap="Faceting horizontally by drv (top); Faceting vertically by cyl (bottom left); Faceting in both directions, with both variables (bottom right)", fig.width=6, fig.height=4}
...
library(dplyr)
library(forcats)  # Load the forcats library to help you rename the factor levels

# Make a modified copy of the original data called `mpg_mod1`
mpg_mod1 <- 
  mpg %>% 
  # Rename 4 to 4wd, f to Front, r to Rear
  mutate(drv = fct_recode(
    drv,
    `4wd` = "4", 
    Front = "f",
    Rear = "r"
...
# Plot the new data
ggplot(mpg_mod1, aes(x = displ, y = hwy)) + 
  geom_point() + 
  facet_grid(drv ~ .)
```
...
# Make a modified copy of the original data called `mpg_mod`
mpg_mod2 <- 
  mpg %>% 
  # Rename 4 to 4wd, f to Front, r to Rear
  mutate(drv = fct_recode(
...
ggplot(mpg_mod2, aes(x = displ, y = hwy)) + 
  geom_point() +
