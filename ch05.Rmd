...
With the `heightweight` data set, we'll make a linear model for each of the levels of `sex`, and put those model objects in a list. The model building is done with a function, `make_model()`, defined below. If you pass `make_model()` a data frame, the function will return an lm object. The model can be customized for your data.
...
Now that we have the list of model objects, we can run the `predictvals()` defined in \@ref(RECIPE-SCATTER-FITLINES-MODEL) to get predicted values from each model, using the `ldply()` function:
