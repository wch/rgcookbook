...
the trends aren't immediately obvious just by looking at it.
...
another. When this happens, you'll probably want to summarize the
data before displaying it. We'll also see how to do that in this
chapter.
...
Use geom_point(), andmap one variable to x and one to y.

In the heightweight data set, there are a number of columns, but
we'll only use two in this example
(Figure \@ref(fig:FIG-SCATTER-BASIC)):
...
(\#21), as seen in Figure \@ref(fig:FIG-SCATTER-BASIC-SHAPE-SIZE)
(left):
...
(Figure \@ref(fig:FIG-SCATTER-BASIC-SHAPE-SIZE), right):
...
> cases (see Figure \@ref(fig:Fig_5-3_shape16_shape19)). See
...
set, there are many columns, but we'll only use three of them in
this example:
...
Here, we'll map sex to shape and colour
(Figure \@ref(fig:FIG-SCATTER-SHAPE-COLOR-BOTH), left):
...
can be used with scale_shape\$\$\_\$\$manual(), and other
colors can be used with scale_colour\$\$\_\$\$brewer() or
scale_colour\$\$\_\$\$manual().

This will set different shapes and colors for the groupingvariables
(Figure \@ref(fig:FIG-SCATTER-SHAPE-COLOR-BOTH), right):
...
(Figure \@ref(fig:FIG-SCATTER-SHAPES)), specify the shape in
geom_point():
...
scale_shape\$\$\_\$\$manual() to change the shapes:
...
Figure \@ref(fig:FIG-SCATTER-SHAPES-CHART) shows the shapes that are
...
It's possible to have the shape represent one variable and the fill
...
a hollow shape). For example, we'll take the heightweight data set
and add another column that indicates whether the child weighed 100
pounds or more (Figure \@ref(fig:FIG-SCATTER-SHAPES-FILL)):
...
set, there are many columns, but we'll only use four of them in this
example:
...
Figure \@ref(fig:FIG-SCATTER-CONTINUOUS-COLOR-SIZE):
...
accuracy. We aren't very good at perceiving small differences in
...
Figure \@ref(fig:FIG-SCATTER-CONTINUOUS-COLOR-SIZE) have about 36 times
...
mm, with scale_size\$\$\_\$\$continuous(range=c(2, 5)).
However, the point size numbers don't map linearly to diameter or
area, so this still won't give a very accurate representation of the
...
Figure \@ref(fig:FIG-SCATTER-CONTINUOUS-FILL), because the outline sets
...
When we map a continuous variable to an aesthetic, that doesn't
prevent us from mapping a categorical variable to other aesthetics. In
Figure \@ref(fig:FIG-SCATTER-CONTINUOUS-SIZE-CATEGORICAL-COLOR),
we'll map weightLb to size, and also map sex to colour. Because
there is a fair amount of overplotting, we'll make the points 50%
transparent by setting alpha=.5. We'll also use
scale_size\$\$\_\$\$area() to make the area of the points
...
When a variable is mapped to size, it's a good idea to *not* map a
...
Figure \@ref(fig:FIG-SCATTER-BASIC-SHAPE-SIZE) in
[section\_title](#RECIPE_SCATTER_BASIC_SCATTER) demonstrates both of
these solutions.

If there's a high degree of overplotting, there are a number of
possible solutions:
...
The scatter plot in Figure \@ref(fig:FIG-SCATTER-OVERPLOT) contains
...
Figure \@ref(fig:FIG-SCATTER-OVERPLOT-ALPHA). Here, we'll make them
...
Figure \@ref(fig:FIG-SCATTER-OVERPLOT-BIN2D). With the binned
...
By default, stat_bin\$\$\_\$\$2d() divides the space into 30
...
don't vary much in luminosity. In the second version we set the
colors by using scale_fill\$\$\_\$\$gradient() and specifying
the low and high colors. By default, the legend doesn't show an
...
Figure \@ref(fig:FIG-SCATTER-OVERPLOT-BIN2D), right), we can manually
set the range from 0 to the maximum, 6000, using limits
(Figure \@ref(fig:FIG-SCATTER-OVERPLOT-BIN2D), left):
...
rectangles, with stat_binhex()
(Figure \@ref(fig:FIG-SCATTER-OVERPLOT-BINHEX)). It works just like
stat_bin2d(). To use it, you must first install the hexbin
...
Figure \@ref(fig:FIG-SCATTER-OVERPLOT-BINHEX).
...
axes, as shown in Figure \@ref(fig:FIG-SCATTER-OVERPLOT-JITTER). In
these cases, you can randomly *jitter* the points with
position_jitter(). By default the amount of jitter is 40% of
...
Figure \@ref(fig:FIG-SCATTER-OVERPLOT-BOXPLOT). This will convey a
...
since it is stored numerically, ggplot() doesn't know how to group
the data for each box. If you don't tell it how to group the data,
you get a result like the graph on the right in
Figure \@ref(fig:FIG-SCATTER-OVERPLOT-BOXPLOT). To tell it how to group
the data, use aes(group=...). In this case, we'll group by each
...
![Left: grouping into box plots; right: what happens if you don't
...
stat_smooth() and tell it to use method=lm. This instructs it
to fit the data with the lm() (linear model) function. First we'll
save the base plot object in sp, then we'll add different components
to it:
...
(Figure \@ref(fig:FIG-SCATTER-FIT-LM)):
...
by setting colour (Figure \@ref(fig:FIG-SCATTER-FIT-LM), bottom right):
...
data—in fact, it's not even the default. If you add
stat_smooth() without specifying the method, it will use a
loess (locally weighted polynomial) curve, as shown in
Figure \@ref(fig:FIG-SCATTER-FIT-LOESS). Both of these will have the
same result:
...
just passing them to stat_smooth().

Another common type of model fit is a logistic regression. Logistic
regression isn't appropriate for the heightweight data set, but
it's perfect for the biopsy data set in the MASS library. In this
...
numeric values of 0 and 1. We'll make a copy of the biopsy data
...
we'll just look at the relationship of V1 (clump thickness) and the
class of the tumor. Because there is a large degree of overplotting,
we'll jitter the points and make them semitransparent (alpha=0.4),
hollow (shape=21), and slightly smaller (size=1.5). Then we'll add a
fitted logistic regression line
(Figure \@ref(fig:FIG-SCATTER-FIT-LOGISTIC)) by telling
stat_smooth() to use the glm() function with the option
family=binomial:
...
shape, one fit line will be drawn for each group. First we'll make
the base plot object sps, then we'll add the loess lines to it.
We'll also make the points less prominent by making them
...
Notice that the blue line, for males, doesn't run all the way to the
right side of the graph. There are two reasons for this. The first is
that, by default, stat_smooth() limits the prediction to within
...
right-hand image of Figure \@ref(fig:FIG-SCATTER-FIT-GROUP), you must
...
stat_smooth() (with LOESS and no extrapolation) make more sense
than the extrapolated linear predictions, because we don't grow
linearly and we don't grow forever.
...
stat_smooth() to do it for you, as described in
...
allows you to be sure that the model you're using for other
calculations is the same one that you see.

In this example, we'll build a quadratic model using lm() with
ageYear as a predictor of heightIn. Then we'll use the predict()
...
model (as you'll see in Figure \@ref(fig:FIG-SCATTER-FIT-MODEL)):
...
frame can then be passed to geom_line() to draw the fitted
...
To illustrate this, we'll use the biopsy data set from the MASS
library. As we did in [section\_title](#RECIPE_SCATTER_FITLINES),
we'll use V1 to predict class. Since logistic regression uses values
from 0 to 1, while class is a factor, we'll first have to convert
class to 0s and 1s:
...
Finally, we'll make the graph with jittered points and the
fitlogistic line. We'll make the line in a shade of blue by
specifying a color in RGB values, and slightly thicker, with size=1
(Figure \@ref(fig:FIG-SCATTER-FIT-MODEL-LOGISTIC)):
...
The model building is done with a function, make_model(),
...
grouping variable sex, and apply make_model() to each subset.
In this case, the heightweight data will be split into two data frames,
one for males and one for females, and make_model() will be run
...
(Figure \@ref(fig:FIG-SCATTER-FIT-MODEL-MULTI)):
...
As you can see in Figure \@ref(fig:FIG-SCATTER-FIT-MODEL-MULTI-RANGE),
the line for males now extends as far to the right as the line for
females. Keep in mind that extrapolating past the data isn't always
appropriate, though; whether or not it's justified will depend on
...
we'll create a linear model and use the predictvals() function
...
‘.' 0.1 ‘ ' 1 Residual standard error: 2.989 on 234 degrees of
freedom Multiple R-squared: 0.4249, Adjusted R-squared: 0.4225
F-statistic: 172.9 on 1 and 234 DF, p-value: &lt; 2.2e-16
This shows that the *r^2^* value is 0.4249. We'll create a graph and
manually add the text using annotate()
(Figure \@ref(fig:FIG-SCATTER-FIT-MODEL-TEXT)):
...
Instead of using a plain text string, it's also possible to enter
formulas using R's math expression syntax, by setting parse=TRUE:
...
It's possible to automatically extract values from the model object
and build an expression using those values. In this example, we'll
...
Use geom_rug(). Forthis example
(Figure \@ref(fig:FIG-SCATTER-RUG)), we'll use the faithful data
...
(Figure \@ref(fig:FIG-SCATTER-RUG-JITTER)). This helps the viewer see
...
geom_text(). For this example, we'll use the countries data
...
we'll just take the subset of countries that spent more than \$2000
USD per capita:
...
We'll save the basic scatter plot object in sp and add then add
things to it. To manually add annotations, use annotate(), and specify
the coordinates and label (Figure \@ref(fig:FIG-SCATTER-LABEL), left).
...
(Figure \@ref(fig:FIG-SCATTER-LABEL), right), use geom_text()
and map a column that is a factor or character vector to the label
aesthetic. In this case, we'll use Name, and we'll make the font
...
(Figure \@ref(fig:FIG-SCATTER-LABEL-HJUST), left), and to right-justify,
set hjust=1. As was the case with vjust, the labels will still slightly
overlap with the points. This time, though, it's not a good idea to
...
labels move further than shorter ones. It's better to just set hjust
to 0 or 1, and then add or subtract a bit to or from x
(Figure \@ref(fig:FIG-SCATTER-LABEL-HJUST), right):
...
> you'll need to *multiply* the x or y value by a number to shift
...
containing just the labels you want. Here's one way to do that:
first we'll make a copy of the data we're using, then we'll
...
Next, we'll use the %in% operator to find *where* each name that we
...
Then we'll use that Boolean vector to overwrite all the *other*
...
Now we can make the plot (Figure \@ref(fig:FIG-SCATTER-LABEL-SELECT)).
This time, we'll also expand the *x* range so that the text will
fit:
...
Use geom_point() with scale_size\$\$\_\$\$area(). For
this example, we'll use a subset of the countries data set:
...
of the dots (Figure \@ref(fig:FIG-SCATTER-BALLOON), left), which is not
...
In this example we've used a few tricks to add the text labels under
...
doesn't jump out at the viewer and overwhelm the perceptual impact
...
For this example, we'll use a subset of the countries data set.
We'll pull out the data for the year 2009, and keep only the columns
that are relevant:
...
66.8 NA 52.2
To make the scatter plot matrix (Figure \@ref(fig:FIG-SCATTER-SPLOM)),
we'll use columns 2 through 5—using the Name column wouldn't
...
We didn't use ggplot2 here because it doesn't make scatter plot
...
plot, we'll define the function panel.cor. This will also show
higher correlations in a larger font. Don't worry about the details
...
it's more convenient, you can simply open that help page, then copy
and paste. The last line of this version of the panel.cor function is
slightly modified, however, so that the changes in font size aren't
as extreme as with the original.

Now that we've defined these functions we can use them for our
scatter plot matrix, by telling pairs() to use panel.cor for the upper
panels and panel.hist for the diagonal panels.

We'll also throw in one more thing: panel.smooth for the lower
panels, which makes a scatter plot and adds a LOWESS smoothed line, as
shown in Figure \@ref(fig:FIG-SCATTER-SPLOM-PANELS). (LOWESS is slightly
...
previous panel functions, this one isn't in the pairs help page):
...
We'll also use small points in the visualization, so that we can
distinguish them a bit better
(Figure \@ref(fig:FIG-SCATTER-SPLOM-PANELS2)). This is done by setting
pch=".":
