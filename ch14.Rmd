...
The patchwork package can be used to combine several ggplot2 plots into the one graphic. Once you use patchwork to stich your plots together, you can output the combined graphic into any number of formats, as outlined in the preceeding recipes.

As of this writing, the patchwork package is not available on CRAN, so you need to install it using the devtools package. (The latest [installation instructions](https://github.com/thomasp85/patchwork) can be found on the project's GitHub page).
...
```{r FIG-OUTPUT-PATCHWORK-HEIGHT, fig.show="hold", fig.cap="Using `plot_layout()` to specify the heights of each plot "}
plot1 + plot2 +
