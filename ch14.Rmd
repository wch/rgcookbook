...
The patchwork package can be used to combine several plots - specifically ggplots - into the same graphic. Once you use patchwork to stich your plots together, you can output the combined graphic into any number of formats, as outlined in the preceeding recipes.

You will first need to install the package using the following command (the latest [installation instructions](https://github.com/thomasp85/patchwork) can be found on github):
...
```{r FIG-OUTPUT-PATCHWORK, fig.show="hold", fig.cap="Combining two plots together using `patchwork()`"}
library(patchwork)
...
Patchwork also allows you to determine how you want to lay out the ggplots in relation to each other, by adding a `plot_layout()` call. You can use this call to determine the number of columns you want the pplots to be arranged in, and the size of the plots (Figure \@ref(fig:FIG-OUTPUT-PATCHWORK-LAYOUT)):

```{r FIG-OUTPUT-PATCHWORK-LAYOUT, fig.show="hold", fig.cap="Using `plot_layout()` to specify that the plots should be arranged in 2 columns (left); specifying the heights of each plot (right)"}
plot1 + plot2 + 
  plot_layout(ncol = 2)
