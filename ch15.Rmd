...
Note that you can also use the `class()` function to get information about the data set object itself (rather than about each of the variables contained within the dataframe), or to get information about a specific variable within a dataframe:

```{r}
# Get information about the overall data frame
class(ToothGrowth)
# Get information about a particular variable
class(ToothGrowth$len)
```

Whereas `str()` and `summary()` give you information about both the type of object as well as some other descriptive information, `class()` will only give you information about the object type.
...
Use `mutate()` to add a new column and assign a value to the new column. Note that this returns a new data frame, so if you want to save the output you'll need to save over the original, as in these examples.
...
Note that the length of the vector being added to the data frame must be the same number of rows as the data frame. In the example above we created a new vector that had 60 rows by repeating the values c(1, 2) thirty times.

### Discussion

Each column of a data frame is a vector. R handles columns within data frames slightly differently from standalone vectors because all the columns in a data frame have the same length.
...
Use `select()` and specify the columns you want to drop by using `-` (a minus sign). Note that this returns a new data frame, so if you want to save the output you'll need to save over the original, as in these examples.
...
# is equivalent to the following code, which specifies that we only want to keep dose
tg_mod <-
  ToothGrowth %>% 
  select(dose)
```

One note is to make sure that you don't use `select()` to both drop and keep columns within the same command. If you do, only the first command will be observed. You can try this out by comparing the results of the code below

```{r eval=FALSE}
# Note that these two pieces of code do not give you the same thing! 

# This code acts as if the last line reads as `select(-len)`
tg_mod <-
  ToothGrowth %>% 
  select(-len, supp)

# This code acts as if the last line reads as `select(supp)`
tg_mod <-
  ToothGrowth %>% 
  select(supp, -len)
```
...
Use the `rename()` function. This returns a new data frame, so if you want to save the output you'll need to save over the original, as in these examples.
...
You can rename multiple columns within the same `rename()` function:

```{r eval=FALSE}
tg_mod <-
  ToothGrowth %>% 
  rename(length = len, supplement_type = supp)
```
...
Use the `select()` function. In the example below, the original order of the columns was first `len`, followed by `supp`, and `dose`. 
...
The new data frame will contain the variables you have specified within `select()`, in the order that you have specified. Note that `select()` returns a new data frame, so if you want to save the output you'll need to save over the original.

### Discussion

If you are only reordering a few variables and want to keep the rest of the variables, you can include `everything()` to indicate that you want the new data frame to still keep all of the remaining variables:

```{r}
# We can use `everything()` instead of having to type out all of the remaining variable names
ToothGrowth %>%
...
Use `filter()` to get the rows, and `select()` to get out the columns you want. These operations can be chained together using the `%>%` operator. These functions return new data frame, so if you want to save the output you'll need to save over the original.

We'll use the `climate` data set for the examples here:

```{r}
library(gcookbook) # Load the gcookbook package to access the climate data set
climate
```
...
What if you now only want to keep the two columns named `Year` and `Anomaly10y`? As we saw in \@ref(RECIPE-DATAPREP-DELETE-COL), we can use `select()`:
...
These operations can be chained together using the `%>%` operator, such that the two examples above can be combined as follows:
...
Use `fct_relevel()` from the forcats package. This returns a new factor, so if you want to save the output you'll need to save over the original, as in the examples below.

In this example, we'll create a factor that initially has the wrong ordering:
...
library(forcats)
sizes_ordered <- fct_relevel(sizes, "small", "medium", "large")
sizes_ordered
```
...
```{r eval=FALSE}
fct_relevel(sizes, levels = rev(levels(sizes)))
```
...
Use `fct_reorder()` from the forcats package with the factor that has levels to reorder, the values to base the reordering on, and a function that aggregates the values. This returns a new factor, so if you want to save the output you'll need to save over the original.
...
The usefulness of `fct_reorder()` might not be obvious from just looking at the raw output. Figure \@ref(fig:FIG-DATAPREP-FACTOR-REORDER-VALUE) shows two graphs made with `fct_reorder()`. In these graphs, the order in which the items appear is determined by their values:

```{r FIG-DATAPREP-FACTOR-REORDER-VALUE, echo=FALSE, fig.show="hold", fig.cap="Original data (left); Reordered by the mean of each group (right)", fig.height=2.5, fig.width=3}
ggplot(InsectSprays, aes(spray, count)) +
  geom_boxplot()

ggplot(InsectSprays, aes(fct_reorder(spray, count, .fun = mean), count)) +
  geom_boxplot()
```
...
Use `fct_recode()` from the forcats package. This returns a new factor, so if you want to save the output you'll need to save over the original.
...
To remove them, use `fct_drop` from the forcats package`. This returns a new factor, so if you want to save the output you'll need to save over the original.

```{r}
sizes_mod <- fct_drop(sizes)
sizes_mod
```

### Discussion

The `fct_drop()` function preserves the order of factor levels. You can use the `only` parameter to keep particular levels.
...
Creating a New Variable Based on an Existing Categorical Variable {#RECIPE-DATAPREP-RECODE-CATEGORICAL}
---------------------------------------------------------------

### Problem

You want to create a new variable based on the values of an existing categorical variable.

### Solution

Use the `mutate()` function from the dplyr package, combined with the `ifelse()` function.

For the examples here, we'll add a new variable to the `PlantGrowth` data set. We'll call this new variable `treatment`, whose values are based on the `group` variable. If `group` is `"ctrl"`, the new value of `treatment` will be `"no"`; if `group` is `"trt1"` or `"trt2"`, the new value for `treatment` will be `"yes"`.

```{r}
library(dplyr)

pg_mod <- PlantGrowth %>%
  mutate(treatment = ifelse(group == "ctrl", "no", "yes"))
pg_mod
```

Since the input was a factor, it returns a factor. If you want to get a character vector instead, use `as.character()`:

```{r}
pg_mod <- PlantGrowth %>%
  mutate(trt = as.character(ifelse(group == "ctrl", "no", "yes")))
```

### Discussion

Note that the `ifelse()` function used in the two code chunks above will assign `"yes"` to `treatment` for any value of `group` that isn't `"ctrl"`, including any value that isn't `"trt1"` or `"trt2"`. To specify that only rows where `group` is `"trt1"` or `"trt2"` should be marked as `"yes"`, we can embed another `ifelse()` function and use the `%in%` operator:
```{r, eval=FALSE}
pg_mod <- PlantGrowth %>%
  mutate(treatment = ifelse(group == "ctrl", "no", ifelse(
    group %in% c("trt1", "trt2"), "yes", NA))
  )
```

The code above tells us that if `group == "ctrl"`, `"no"` will be assigned to `treatment`. If `group` does not equal `"ctrl"`, we go to the second `ifelse()`, which tells us that if `group` matches any value in `c("trt1", "trt2")`, `"yes"` will be assigned to `treatment`. If the value of `group` fails both of these condition, `NA` will be assigned to `treatment`.
...
Creating New Columns using Calculations Based on Existing Columns  {#RECIPE-DATAPREP-CALCULATE}
-----------------------
...
Use `mutate()` from the dplyr package. For this example, we'll use a copy of the `heightweight` data set:
...
Calculating New Columns based on a Grouping Column {#RECIPE-DATAPREP-CALCULATE-GROUP}
-------------------------------
...
```{r}
library(MASS)  # Load the MASS package to access the cabbages data set
library(dplyr)
...
Note that this returns a new data frame, so if you want to save the output you'll need to save over the original.
