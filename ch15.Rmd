...
real-world data, this usually isnRSQUOt the case: youRSQUOll have to
clean up and restructure the data before you can visualize it.

Data sets in R are most often stored in data frames. TheyRSQUOre
...
HereRSQUOs the heightweight data set:
...
itRSQUOs structured by using the str() function:
...
### Problem {#_problem_139}

You want to add a column to a data frame.

### Solution {#_solution_139}
...
### Discussion {#_discussion_131}

Each LDQUOcolumnRDQUO of a data frame is a vector or factor. R handles
...
Finally, itRSQUOs also possible to pick out rows and columns by their
...
possible. It makes the code easier to understand when youRSQUOre
...
### Discussion {#_discussion_137}

The usefulness of reorder() might not be obvious from just looking at
the raw output. [figure\_title](#FIG_DATAPREP_FACTOR_REORDER_VALUE)
...
[figure\_title](#FIG_DATAPREP_FACTOR_REORDER_VALUE), the boxes are
...
increase strictly from left to right. ThatRSQUOs because with this
...
[figure\_title](#FIG_DATAPREP_FACTOR_REORDER_VALUE), we used the
...
ItRSQUOs also possible to rename factor levels by position, but this is
somewhat inelegant:
...
ItRSQUOs safer to rename factor levels by name rather than by position,
...
### Problem {#_problem_149}

You want to recode a categorical variable to another variable.

### Solution {#_solution_149}

For the examples here, weRSQUOll use a subset of the PlantGrowth data
set:
...
In this example, weRSQUOll recode the categorical variable group into
...
ItRSQUOs also possible to combine two columns into one usingthe
...
PlantGrowth\[c(1,2,11,21,22), \] pg
weight group 4.17 ctrl 5.58 ctrl 4.81 trt1 6.31 trt2 5.12 trt2
In this example, weRSQUOll recode the continuous variable weight into a
...
### Discussion {#_discussion_142}

For three categories we specify four bounds, which can include Inf and
-Inf. If a data value falls outside of the specified bounds, itRSQUOs
...
the left and *closed* on the right. In other words, they donRSQUOt
...
### Discussion {#_discussion_144}

LetRSQUOs take a closer look at the cabbages data set. It has two
...
YouRSQUOll often want to do separate operations like this for each
...
[figure\_title](#FIG_DATAPREP_TRANSFORM_GROUP):
...
clear, weRSQUOll use the summarise() function instead:
...
variables. ItRSQUOs also possible to get more than one summary value by
specifying more calculated columns. Here weRSQUOll summarize each Cult
...
Cult Date Weight VitC c39 d16 3.18 50.3 c39 d20 2.80 49.4 c39 d21 2.74
54.8 c52 d16 2.26 62.5 c52 d20 3.11 58.9 c52 d21 1.47 71.8
ItRSQUOs possible to do more than take the mean. You may, for example,
...
output. LetRSQUOs see what happens if we sprinkle a few NAs into HeadWt:
...
excluded from the count. The length() function doesnRSQUOt have an na.rm
...
cause problems when making graphs. To illustrate, weRSQUOll remove all
entries that have levels c52 and d21. The graph on the left in
[figure\_title](#FIG_DATAPREP_SUMMARIZE_MISSING_COMBO) shows what
happens when thereRSQUOs a missing combination in a bar graph:
...
([figure\_title](#FIG_DATAPREP_SUMMARIZE_MISSING_COMBO), right), give
...
stat\$\$\_\$\$summary() to calculate means and overlay them on a graph.
...
### Discussion {#_discussion_146}

Another method is to calculate the standard error in the call ddply.
ItRSQUOs not possible to refer to the sd and n columns inside of the
ddply call, so weRSQUOll have to recalculate them to get se. This will
...
interval means that thereRSQUOs a 95% chance that the true population
mean is within the interval (actually, it doesnRSQUOt mean this at all,
...
This isnRSQUOt a problem; it just indicates that it couldnRSQUOt
...
### Problem {#_problem_155}

You want to convert a data frame from LDQUOwideRDQUO format to
LDQUOlongRDQUO format.

### Solution {#_solution_155}
...
If you donRSQUOt want to use all the non-ID columns as measure
...
ItRSQUOs also possible to use more than one column as the ID variables:
...
in\_spring alive 31
Some data sets donRSQUOt come with a column with an ID variable. For
...
wonRSQUOt be able to tell how the values are meant to be paired
...
### Problem {#_problem_156}

You want to convert a data frame from LDQUOlongRDQUO format to
LDQUOwideRDQUO format.

### Solution {#_solution_156}

Use the dcast() function from the reshape2 package. In this example,
weRSQUOll use the plum data set, which is in a long format:
...
for source values. For example, we can LDQUOmoveRDQUO values in the
...
LDQUOmoved to the topRDQUO). This is done with a formula where the ID
