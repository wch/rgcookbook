...
Use facet_grid() or facet_wrap(), and specify the
variables on which to split.

With facet_grid(), you can specify a variable to split the data
...
With facet_wrap(), the subplots are laid out horizontally and
...
With facet_wrap(), the default is to use the same number of
...
ranges), or by adding geom_blank() (to expand the ranges).

### See Also

See Recipe \@ref(RECIPE-BAR-GRAPH-DOT-PLOT) for an example of faceting with
...
With facet_grid() but not facet_wrap(), at this time),
it's possible to use a labeller function to set the labels. The
labeller function label_both() will print out both the name of
...
See Recipe \@ref(RECIPE-DATAPREP-FACTOR-RENAME) for more on renaming factor
levels. If the faceting variable is not a factor but a character vector,
changing the values is somewhat different. See
Recipe \@ref(RECIPE-DATAPREP-CHARACTER-RENAME) for information on renaming
...
Recipe \@ref(RECIPE-APPEARANCE-THEME) and
