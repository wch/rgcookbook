...
TODO: Fix warning about `seq.default`

```{r eval=FALSE}
library(gcookbook)  # Load the gcookbook package to access the cabbage_exp data set 
...
```{r FIG-COLORS-MAPPING, echo=FALSE, fig.show="hold", message=FALSE, fig.cap="Mapping a variable to `fill` (left); Mapping a variable to `colour` for points (right)"}
library(gcookbook)  # Load the gcookbook package to access the cabbage_exp data set 
...
```{r FIG-COLORS-MAPPING-FACTOR, fig.show="hold", fig.cap="Mapping to colour with a continuous variable converted to a factor"}
...
+-------------------------+---------------------------+-----------------------+
| `scale_fill_viridis()`  | `scale_colour_viridis()   | Viridis palettes,     |
|                         |                           | designed to be        |
|                         |                           | colorful, uniform,    |
|                         |                           | robust to colorblindness|
|                         |                           | and grey scale printing|
+-------------------------+---------------------------+-----------------------+
...
library(gcookbook)  # Load the gcookbook package to access the uspopage data set 
library(viridis)  # Load the viridis package to access the viridis palette
...
```{r FIG-COLORS-PALETTE-LIGHTNESS, fig.show="hold", fig.cap="Points with default lightness (left); With lightness set to 45 (right)", fig.width=4}
...
The viridis package contains a set of color scales that are each designed to span as wide a palette as possible, making for colorful and pretty color scales that also make it easier to see differences in your data. These scales are also designed to be perceptually uniform, printable in grey scale, and easier to read by those with colorblindness. Here is an example from this [introduction page to viridis](https://cran.r-project.org/web/packages/viridis/vignettes/intro-to-viridis.html) (Figure \@ref(fig:FIG-COLORS-PALETTE-VIRIDIS)):

```{r FIG-COLORS-PALETTE-VIRIDIS, echo=FALSE, fig.cap="Example of viridis color palette", fig.height=8, fig.width=8}
...
```{r FIG-COLORS-PALETTE-BREWER, fig.cap="All the ColorBrewer palettes", fig.height=6, fig.width=8}
library(RColorBrewer)
display.brewer.all()
```
The ColorBrewer palettes can be selected by name. For example, this will use the "Greens" palette (Figure \@ref(fig:FIG-COLORS-PALETTE-BREWER-NAME)):

```{r FIG-COLORS-PALETTE-BREWER-NAME, fig.cap="Using a named ColorBrewer palette", fig.width=4}
hw_splot + 
...
```{r FIG-COLORS-PALETTE-GREY, fig.show="hold", fig.cap="Using the default grey palette (left); A different grey palette (right)", fig.width=4}
hw_splot + 
  scale_colour_grey()
...
You may want to manually select colors based on the color scales in the viridis package, as described in Recipe \@ref (#RECIPE-COLORS-PALETTE-DISCRETE). You can do so by specifying the number of discrete categories you have within the `viridis()` function. This will generate the RBG hexadecimals. You can similarly generate the RBG values for the other color scales in the viridis package: "magma", "plasma", "inferno", and "cividis". 
...
Use the color scales in the viridis package, as described in Recipe \@ref (#RECIPE-COLORS-PALETTE-DISCRETE) and Recipe \@ref (#RECIPE-COLORS-PALETTE-DISCRETE-MANUAL) and shown in Figure \@ref(fig:FIG-COLORS-PALETTE-DISCRETE-COLORBLIND-VIRIDIS):

```{r FIG-COLORS-PALETTE-DISCRETE-COLORBLIND-VIRIDIS, fig.show="hold", fig.cap="A plot with the colorblind-friendly viridis palette", fig.width=4.5}
library(gcookbook)  # Load the gcookbook package to access the uspopage data set 
...
You can also use the palette defined below (`cb_palette`) with `scale_fill_manual()` (Figure \@ref(fig:FIG-COLORS-PALETTE-DISCRETE-COLORBLIND-MANUAL)):

```{r FIG-COLORS-PALETTE-DISCRETE-COLORBLIND-MANUAL, fig.show="hold", fig.cap="Manually specifying a colorblind-friendly palette", fig.width=4.5}

# Creating a colorblind-friendly palette manually
...
```{r FIG-COLORS-PALETTE-DISCRETE-COLORBLIND-CHART, echo=FALSE, fig.cap="Colorblind palette with RGB values",  fig.width=2}
...
More about [viridis](https://cran.r-project.org/web/packages/viridis/vignettes/intro-to-viridis.html).

The [source of the palette we manually defined](http://jfly.iam.u-tokyo.ac.jp/color/).
...
```{r FIG-COLORS-PALETTE-CONTINUOUS, fig.show="hold", message=FALSE, fig.cap="(ref:cap-FIG-COLORS-PALETTE-CONTINUOUS)", fig.width=4}
library(gcookbook)  # Load the gcookbook package to access the heightweight data set 
...
cbi <-
  data.frame(Year = interp$x, Anomaly10y = interp$y) %>% 
  mutate(valence = ifelse(Anomaly10y >= 0, "pos", "neg"))
```
