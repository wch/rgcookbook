...
In ggplot2RSQUOs implementation of the grammar of graphics, color is an
...
deficiencies? In this chapter, IRSQUOll address these issues.
...
In ggplot2, thereRSQUOs an important difference between *setting* and
...
In the example here weRSQUOll use the default palette (hue), and a
...
for filled areas, but itRSQUOs a bit light for points and lines. To make
...
In the example here, weRSQUOll manually define colors by specifying
...
the second goes with m. HereRSQUOs how to see the order of factor
levels:
...
ItRSQUOs possible to specify the colors in a different order by using a
named vector:
...
idea what "thistle3" and "seashell" look like), so itRSQUOs often easier
...
color-vision deficiency, so thereRSQUOs a good chance that someone in
...
but keep in mind that the simulation isnRSQUOt perfect. In my informal
...
deficiency viewed the same image and couldnRSQUOt tell the colors apart!
...
In the example here, weRSQUOll specify the colors for a continuous
...
Once weRSQUOve categorized the values as positive or negative, we can
...
If you look closely at the figure, youRSQUOll notice that there are some
...
weRSQUOll make a few adjustments—weRSQUOll make the shaded regions
