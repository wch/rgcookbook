...
(Figure \@ref(fig:FIG-COLORS-SETTING)):
...
Recipe \@ref(RECIPE-LINE-GRAPH-POINT-APPEARANCE).

See Recipe \@ref(RECIPE-COLORS-PALETTE-DISCRETE-MANUAL) for more on
specifying colors.

Mapping Variables to Colors {#RECIPE-COLORS-MAPPING}
---------------------------
...
one of the columns in the data (Figure \@ref(fig:FIG-COLORS-MAPPING)):
...
In the cabbage_exp example, the variable Cultivar is mapped to
fill. The Cultivar column in cabbage_exp is a factor, so
...
(Figure \@ref(fig:FIG-COLORS-MAPPING-FACTOR)):
...
continuous data, see
Recipe \@ref(RECIPE-COLORS-PALETTE-CONTINUOUS). For discrete data,
see Recipes Recipe \@ref(RECIPE-COLORS-PALETTE-DISCRETE) and
Recipe \@ref(RECIPE-COLORS-PALETTE-DISCRETE-MANUAL).
...
+======================+======================+=======================+
| scale_fill\ | scale_colou | Colors evenly spaced  |
| $\$\_\$\$discrete()  | r_discrete( | around the color      |
|                      | )                    | wheel (same as hue)   |
+----------------------+----------------------+-----------------------+
| scale_fill\ | scale_colou | Colors evenly spaced  |
| $\$\_\$\$hue()       | r_hue()     | around the color      |
|                      |                      | wheel (same as        |
|                      |                      | discrete)             |
+----------------------+----------------------+-----------------------+
| scale_fill\ | scale_colou | Greyscale palette     |
| $\$\_\$\$grey()      | r_grey()    |                       |
+----------------------+----------------------+-----------------------+
| scale_fill\ | scale_colou | ColorBrewer palettes  |
| $\$\_\$\$brewer()    | r_brewer()  |                       |
+----------------------+----------------------+-----------------------+
| scale_fill\ | scale_colou | Manually specified    |
| $\$\_\$\$manual()    | r_manual()  | colors                |
+----------------------+----------------------+-----------------------+

: Discrete fill and color scales

In the example here we'll use the default palette (hue), and a
ColorBrewer palette (Figure \@ref(fig:FIG-COLORS-PALETTE-DISCRETE)):
...
With scale_fill\$\$\_\$\$hue(), the colors are taken from
...
Figure \@ref(fig:FIG-COLORS-PALETTE-LIGHTNESS) (right), set the value of
...
Figure \@ref(fig:FIG-COLORS-PALETTE-BREWER):
...
(Figure \@ref(fig:FIG-COLORS-PALETTE-BREWER-NAME)):
...
range, as shown in Figure \@ref(fig:FIG-COLORS-PALETTE-GREY).
...
See Recipe \@ref(RECIPE-LEGEND-REVERSE) for more information about reversing
the legend.
...
values with scale_colour\$\$\_\$\$manual()
(Figure \@ref(fig:FIG-COLORS-PALETTE-DISCRETE-MANUAL)). The colors can
...
For fill scales, use scale_fill\$\$\_\$\$manual() instead.
...
Use the palette defined here (cb_palette)with
scale_fill\$\$\_\$\$manual()
(Figure \@ref(fig:FIG-COLORS-PALETTE-DISCRETE-COLORBLIND)):
...
Figure \@ref(fig:FIG-COLORS-PALETTE-DISCRETE-COLORBLIND-CHART).
...
(Figure \@ref(fig:FIG-COLORS-PALETTE-CONTINUOUS)). The colors can be
...
For fill scales, use scale_fill\$\$\_\$\$xxx() versions
...
| Fill scale           | Color scale          | Description           |
+======================+======================+=======================+
| scale_fill\ | scale_colou | Two-color gradient    |
| $\$\_\$\$gradient()  | r_gradient( |                       |
|                      | )                    |                       |
+----------------------+----------------------+-----------------------+
| scale_fill\ | scale_colou | Gradient with a       |
| $\$\_\$\$gradient2() | r_gradient2 | middle color and two  |
|                      | ()                   | colors that diverge   |
|                      |                      | from it               |
+----------------------+----------------------+-----------------------+
| scale_fill\ | scale_colou | Gradient with *n*     |
| $\$\_\$\$gradientn() | r_gradientn | colors, equally       |
|                      | ()                   | spaced                |
+----------------------+----------------------+-----------------------+
...
Recipe \@ref(RECIPE-DATAPREP-RECODE-CONTINUOUS).
...
Figure \@ref(fig:FIG-COLORS-AREA-VALUE):
...
(Figure \@ref(fig:FIG-COLORS-AREA-VALUE-INTERPOLATED)). This time
