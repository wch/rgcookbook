...
p +
  ...
  annotate(
    "text",
    x = mean(range(faithful$eruptions)), y = -Inf,
    vjust = -0.4,
    label = "Bottom middle"
  )
```
...
p +
  annotate(
    "rect",
    xmin = 1950, xmax = 1980, ymin = -1, ymax = 1,
    alpha = .1,fill = "blue"
  )
```
...
To highlight one or more items, create a new column in the data and map it to the color. In this example, we'll create a copy of the PlantGrowth data set called `pg_mod` and create a new column, `hl`, which is set to `no` if the case was in treatment group 1, and set to `yes` if the case was in treatment group 2:

```{r}
library(dplyr)

pg_mod <- PlantGrowth %>%
  mutate(hl = ifelse(group == "trt2", "yes", "no"))
```
