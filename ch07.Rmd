...
Displaying just your data usually isnRSQUOt enough—thereRSQUOs all sorts
...
### Solution {#_solution_56}

Use annotate() and a text geom ([figure\_title](#FIG_ANNOTATE_TEXT)):
...
[figure\_title](#FIG_ANNOTATE_TEXT_PROPERTIES):
...
Be careful not to use geom\$\$\_\$\$text() when you want to add
individual text objects. While annotate(geom="text") will add a single
text object to the plot, geom\$\$\_\$\$text() will create many text
objects based on the data, as discussed in
[???](#RECIPE_SCATTER_LABELS).

If you use geom\$\$\_\$\$text(), the text will be heavily overplotted on
...
In [figure\_title](#FIG_ANNOTATE_TEXT_GEOM_OVERPLOT), each text label is
...
[figure\_title](#FIG_ANNOTATE_TEXT_EDGE). You will also need to adjust
...
### Solution {#_solution_57}

Use annotate(geom="text") and set parse=TRUE
([figure\_title](#FIG_ANNOTATE_TEXT_MATH)):
...
expression. Bear in mind that, in RRSQUOs syntax for mathematical
expressions, you canRSQUOt simply put a variable right next to another
without something else in between. To display two variables next to each
other, as in [figure\_title](#FIG_ANNOTATE_TEXT_MATH_MIX), put a
...
### Solution {#_solution_58}

For horizontal and vertical lines, use geom\$\$\_\$\$hline() and
geom\$\$\_\$\$vline(), and for angled lines, use geom\$\$\_\$\$abline()
([figure\_title](#FIG_ANNOTATE_LINES)). For this example, weRSQUOll use
...
Here weRSQUOll take the average height for males and females and store
it in a data frame, hw\$\$\_\$\$means. Then weRSQUOll draw a horizontal
line for each, and set the linetype and size
([figure\_title](#FIG_ANNOTATE_LINES_MULTI)):
...
If one of the axes is discrete rather than continuous, you canRSQUOt
...
([figure\_title](#FIG_ANNOTATE_LINES_FACTOR)):
...
> annotations. Instead of using the annotate() function, weRSQUOve used
> geom\$\$\_\$\$hline() and friends. This is because previous versions
> of ggplot2 didnRSQUOt have the annotate() function. The line geoms had
...
### Solution {#_solution_59}

Use annotate("segment"). In this example, weRSQUOll use the climate data
set and use a subset of data from the Berkeley source
([figure\_title](#FIG_ANNOTATE_SEGMENT)):
...
### Discussion {#_discussion_54}

ItRSQUOs possible to add arrowheads or flat ends to the line segments,
using arrow() from the grid package. In this example, weRSQUOll do both
([figure\_title](#FIG_ANNOTATE_SEGMENT_ARROWS)):
...
### Solution {#_solution_60}

Use annotate("rect") ([figure\_title](#FIG_ANNOTATE_RECT)):
...
### Discussion {#_discussion_55}

Each layer is drawn in the order that itRSQUOs added to the ggplot
object, so in the preceding example, the rectangle is drawn on top of
the line. ItRSQUOs not a problem in that case, but if youRSQUOd like to
...
parameters. In this case, geom\$\$\_\$\$rect() requires min and max
values for x and y.
...
### Solution {#_solution_61}

To highlight one or more items, create a new column in the data and map
it to the color. In this example, weRSQUOll create a new column, hl, and
...
Then weRSQUOll plot it with manually specified colors and with no legend
([figure\_title](#FIG_ANNOTATE_HIGHLIGHT)):
...
### Solution {#_solution_62}

Use geom\$\$\_\$\$errorbar andmap variables to the values for ymin and
ymax. Adding the error bars is done the same way for bar graphs and line
graphs, as shown in [figure\_title](#FIG_ANNOTATE_ERRORBAR_BASIC)
...
### Discussion {#_discussion_57}

In this example, the data already has values for the standard error of
the mean (se), which weRSQUOll use for the error bars (it also has
values for the standard deviation, sd, but weRSQUOre not using that
here):
...
width=.2. ItRSQUOs best to play around with this to find a value that
looks good. If you donRSQUOt set the width, the error bars will be very
wide, spanning all the space between items on the x-axis.

For a bar graph with groups of bars, the error bars must also be
*dodged*; otherwise, theyRSQUOll have the exact same *x* coordinate and
wonRSQUOt line up with the bars. (See
[???](#RECIPE_BAR_GRAPH_GROUPED_BAR) for more information about grouped
bars and dodging.)

WeRSQUOll work with the full cabbage\$\$\_\$\$exp data set this time:
...
The default dodge width for geom\$\$\_\$\$bar() is 0.9, and youRSQUOll
have to tell the error bars to be dodged the same width. If you
donRSQUOt specify the dodge width, it will default to dodging by the
width of the error bars, which is usually less than the width of the
bars ([figure\_title](#FIG_ANNOTATE_ERRORBAR_GROUP_BAR)):
...
> position=position\$\$\_\$\$dodge(), in the first version. But to pass
> a specific value, we have to spell it out, as in
> position\$\$\_\$\$dodge(0.9).
...
![Error bars on a line graph, dodged so they donRSQUOt
...
### Solution {#_solution_63}

Create a new data frame with the faceting variable(s), and a value to
use in each facet. Then use geom\$\$\_\$\$text() with the new data frame
([figure\_title](#FIG_ANNOTATE_FACET)):
...
### Discussion {#_discussion_58}

This method can be used to display information about the data in each
facet, as shown in [figure\_title](#FIG_ANNOTATE_FACET_MATH). For
example, in each facet we can show linear regression lines, the formula
for each line, and the *r^2^* value (). To do this, weRSQUOll write a
...
*r^2^* value. Then weRSQUOll use ddply() to apply that function to each
group of the data:
...
itRSQUOs possible to do something simpler, by using ddply() with the
...
Text geoms arenRSQUOt the only kind that can be added individually for
...
instead of having ggplot2 do it for you with stat\$\$\_\$\$smooth(), see
